...
function bootstrapCanISellItPage() {
    ...
    function renderAssessment(assessment) {
        ...
        const reviewBrief = assessment.complianceReviewBrief || { overall: 'Unknown', evidence: [], actions: [], costTiming: { status: 'Unknown', detail: 'Human scoping required.' }, professionalConfirmation: [] };
        const evidenceMatrix = reviewBrief.evidence.map((item) => `<li class="sell-review-status--${escapeHtml(item.status.toLowerCase().replaceAll(' ', '-'))}"><strong>${escapeHtml(item.label)}</strong><span>${escapeHtml(item.status)}</span><small>${escapeHtml(item.note)}</small></li>`).join('');
        const reviewActions = reviewBrief.actions.map((item) => `<li>${escapeHtml(item)}</li>`).join('');
        const professionalConfirmation = reviewBrief.professionalConfirmation.length
            ? reviewBrief.professionalConfirmation.map((item) => `<li>${escapeHtml(item)}</li>`).join('')
            : '<li>No additional specialist-confirmation item has been identified yet.</li>';
        ...
        const reviewDetails = () => ({
            exactModel: document.getElementById('sell-review-model')?.value || '',
            productNotes: document.getElementById('sell-review-product-notes')?.value || '',
            power: document.getElementById('sell-review-power')?.value || '',
            wireless: document.getElementById('sell-review-wireless')?.value || '',
            battery: document.getElementById('sell-review-battery')?.value || '',
        ...
        document.getElementById('sell-copy-review-request')?.addEventListener('click', async () => {
            ...
            const prepared = reviewContact();
            try {
                await navigator.clipboard.writeText(prepared.text);
                status.textContent = `Copied. Review it, then send it yourself to ${prepared.email}.`;
            } catch {
                status.textContent = `Copy is unavailable in this browser. No information was sent; email ${prepared.email} manually.`;
            }
        });
