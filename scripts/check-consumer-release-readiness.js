...
function runConsumerReleaseReadiness({ now = Date.now(), maxSourceAgeDays = 370 } = {}) {
    ...
    [
        ...
        ['manual email CTA', /reviewContact\(\)\.mailto/.test(page) && /Prepare \/ Copy request/.test(page)],
        ['local review preparation', /sell-review-model/.test(page) && /sell-review-battery/.test(page) && /reviewDetails/.test(page)],
        ['review service boundary', /not certification or legal advice/i.test(page) && /not a certificate issued by TraceWize/i.test(page)],
