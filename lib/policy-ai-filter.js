...
const { normalizeConfidenceScore, normalizeEffectiveStatus } = require('./compliance-audit');
...
const UNIVERSAL_AI_REFINER_SYSTEM_PROMPT = [
    ...
    'effective_status active|pending_effective (use pending_effective only when the notice clearly states a future effective date);',
    'confidence_score number 0.0–1.0 (your calibrated textual relevance to the three target industries and cross-border trade impact).',
    '',
...
function normalizePolicyAiFilter(raw, context = {}) {
    ...
    return {
        ...
        effective_status: normalizeEffectiveStatus(raw?.effective_status, { relevant }),
        confidence_score: normalizeConfidenceScore(raw?.confidence_score, { relevant }),
        method,
...
async function refineWithAI(rawText, sourceConfig = {}, options = {}) {
        ...
        if (fallback === 'keyword') {
            ...
            return normalizePolicyAiFilter({
                ...
                summary_en: relevant
                    ...
                    : '',
                effective_status: 'active',
                confidence_score: relevant ? 0.35 : 0
    ...
    console.log(
        ...
        + `confidence=${normalized.confidence_score} effective=${normalized.effective_status}`
    );
    return normalized;
}
