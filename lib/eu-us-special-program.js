...
(function (root, factory) {
    ...
    function taricDateToIso(value = '') {
        const match = String(value || '').match(/^(\d{2})-(\d{2})-(\d{4})$/);
        return match ? `${match[3]}-${match[2]}-${match[1]}` : '';
    }

    function findSpecificDutyRows(program = {}, hsCode = '', entryDate = '') {
        const normalizedHs = normalizeCnCode(hsCode);
        const normalizedDate = normalizeEntryDate(entryDate);
        if (!normalizedHs) return [];
        return asArray(program.specific_duty_status?.rows).filter((row) => {
            const code = normalizeCnCode(row.goods_code);
            if (!code || !(normalizedHs.startsWith(code) || code.startsWith(normalizedHs))) return false;
            const start = taricDateToIso(row.start_date);
            const end = taricDateToIso(row.end_date);
            return !normalizedDate || ((!start || normalizedDate >= start) && (!end || normalizedDate <= end));
        });
    }

    function selectAutoSpecificDuty(rows = []) {
        const candidates = asArray(rows).filter((row) => row.simple_specific_duty);
        const fingerprints = new Set(candidates.map((row) => JSON.stringify(row.simple_specific_duty)));
        return fingerprints.size === 1 ? candidates[0].simple_specific_duty : null;
    }
    ...
    function resolveProgramTreatment({
        ...
        const specificDutyRows = findSpecificDutyRows(program, hsCode, entryDate);
        const autoSpecificDuty = selectAutoSpecificDuty(specificDutyRows);
        ...
        const eligibility = resolveEligibilityStatus({
            ...
            declarationCodesConfirmed,
            quotaAvailable,
            specificDutyConfirmed: specificDutyConfirmed === true || Boolean(autoSpecificDuty)
        });
        return {
            appliesToRoute: true,
            matched: matches.length > 0,
            ...
            quotaAlert: quotaRows[0]?.status || 'unknown',
            quotaLastChecked: program.quota_status?.checked_at || '',
            specificDutyRows,
            autoSpecificDuty,
            specificDutyLastChecked: program.specific_duty_status?.checked_at || '',
