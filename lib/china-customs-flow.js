...
const CSV_HEADER_ALIASES = {
    month: ['month', 'period', '统计月份', '统计年月', '数据月份', '数据年月', '年月', '月份'],
    industry: ['industry', 'industry_id', 'category', '行业', '行业类别', '行业分类', '产品类别', '商品类别'],
    hs_code: ['hs_code', 'hs', 'hscode', 'commodity_code', '商品编码', '海关编码', '税则号列', '税则号', '商品编号'],
    product_name: ['product_name', 'commodity_name', 'description', '商品名称', '商品中文名称', '品名'],
    ...
    trade_value_usd: ['trade_value_usd', 'value_usd', 'amount_usd', '美元', '金额美元', '美元金额', '进出口金额美元', '进出口额美元'],
...
// Longest prefixes win so narrower product groups do not leak into their parent category.
const HS_INDUSTRY_PREFIXES = [
    ['854232', 'memory'],
    ['854143', 'solar'],
    ['854142', 'solar'],
    ['854140', 'solar'],
    ['847150', 'computing'],
    ['8471', 'computing'],
    ['8541', 'semiconductor_ai'],
    ['8542', 'semiconductor_ai'],
    ['8517', 'telecom'],
    ['8525', 'telecom'],
    ['8526', 'telecom'],
    ['8529', 'telecom'],
    ['8507', 'battery_energy'],
    ['847950', 'industrial_automation'],
    ['8479', 'industrial_automation'],
    ['8537', 'industrial_automation'],
    ['9018', 'healthcare_lab'],
    ['9027', 'healthcare_lab'],
    ['9504', 'gaming']
...
function normalizeHsCode(value) {
    const code = String(value || '').trim().replace(/[^0-9]/g, '');
    if (code.length < 4) throw new Error(`Invalid HS code: ${value || '(empty)'}`);
    return code;
}

function industryForHsCode(value) {
    const hsCode = normalizeHsCode(value);
    const match = HS_INDUSTRY_PREFIXES.find(([prefix]) => hsCode.startsWith(prefix));
    if (!match) throw new Error(`HS code ${hsCode} is outside the maintained industry scope`);
    return match[1];
}
...
function findHeaderRow(rows, values = {}) {
    ...
    for (let rowIndex = 0; rowIndex < limit; rowIndex += 1) {
        ...
        const hasIdentity = (indexes.month >= 0 || values.default_month)
            && (indexes.industry >= 0 || indexes.hs_code >= 0 || values.default_industry || values.default_hs_code);
...
function combineSeriesRows(rows = []) {
    ...
    rows.forEach((row) => {
        ...
        const hsCodes = [...new Set([...(existing.hs_codes || []), ...(row.hs_codes || [])])];
        const productNames = [...new Set([...(existing.product_names || []), ...(row.product_names || [])])];
        combined.set(key, {
            ...existing,
            ...row,
            month,
...
function parseOfficialRows(rows, values = {}) {
    ...
    meaningfulRows.slice(rowIndex + 1).forEach((columns, offset) => {
        ...
        const rawIndustry = read('industry') || values.default_industry;
        const rawHsCode = read('hs_code') || values.default_hs_code;
        const industryId = rawIndustry ? normalizeIndustryId(rawIndustry) : industryForHsCode(rawHsCode);
        const hsCode = rawHsCode ? normalizeHsCode(rawHsCode) : undefined;
        const partner = String(read('partner') || values.default_partner || 'WORLD').trim().toUpperCase();
        const productName = String(read('product_name') || '').trim();
        ...
        const signature = JSON.stringify([month, industryId, partner, hsCode || '', imports, exports]);
        if (exactRows.has(signature)) return;
        exactRows.add(signature);
        const key = [month, industryId, partner].join('|');
        ...
        if (hsCode && !current.hs_codes.includes(hsCode)) current.hs_codes.push(hsCode);
        if (productName && !current.product_names.includes(productName)) current.product_names.push(productName);
...
function parseOfficialFile(filePath, values = {}) {
    ...
    const defaults = { ...inferFileDefaults(filePath), ...overrides };
    if (/\.xlsx?$/i.test(filePath)) return parseOfficialWorkbook(fs.readFileSync(filePath), defaults);
    const body = fs.readFileSync(filePath);
    const text = isUtf8(body) ? body.toString('utf8') : new TextDecoder('gb18030').decode(body);
    return parseOfficialExport(text, defaults);
}
...
function coverageDiagnostics(payload = {}, officialPeriodOverride, options = {}) {
    ...
    const requiredPeriods = [...new Set((options.requiredPeriods || [])
        .map((value) => normalizeMonth(value, 'required period')))].sort();
    const periodsToCheck = requiredPeriods.length ? requiredPeriods : (targetPeriod ? [targetPeriod] : []);
    const rowsByCombination = new Map(sourceRows.map((row) => [`${row.month}|${row.industry_id}`, row]));
    const missingCombinations = periodsToCheck.flatMap((month) => INDUSTRIES.flatMap(({ id }) => {
        const row = rowsByCombination.get(`${month}|${id}`);
        return [
            row?.imports_value_usd === null || row?.imports_value_usd === undefined
                ? { month, industry_id: id, direction: 'imports', key: `${month}:${id}:imports` }
                : null,
            row?.exports_value_usd === null || row?.exports_value_usd === undefined
                ? { month, industry_id: id, direction: 'exports', key: `${month}:${id}:exports` }
                : null
        ].filter(Boolean);
    }));
    const requiredCombinationCount = periodsToCheck.length * INDUSTRIES.length * 2;
    const batchComplete = requiredCombinationCount > 0 && missingCombinations.length === 0;
    return {
        ...
        required_periods: periodsToCheck,
        required_combination_count: requiredCombinationCount,
        completed_combination_count: requiredCombinationCount - missingCombinations.length,
        missing_combinations: missingCombinations,
        batch_complete: batchComplete
    };
}

function buildCoveragePlan(payload = {}, officialPeriodOverride, options = {}) {
    const diagnostics = coverageDiagnostics(payload, officialPeriodOverride, options);
    const rows = new Map(
        (payload.series || [])
            .filter((row) => row.source_id === SOURCE_ID)
            .map((row) => [`${row.month}|${row.industry_id}`, row])
    );
    const requiredRows = diagnostics.required_periods.flatMap((month) => INDUSTRIES.map(({ id, label }) => {
    ...
    return {
        ...
        required_periods: diagnostics.required_periods,
        required_direction_count: diagnostics.required_combination_count,
        completed_direction_count: diagnostics.completed_combination_count,
        completed_industry_count: requiredRows.filter((row) => row.status === 'complete').length,
        missing_direction_count: diagnostics.missing_combinations.length,
        missing_combinations: diagnostics.missing_combinations,
        complete: diagnostics.batch_complete,
...
function normalizeRow(row) {
    ...
    return {
        ...
        hs_codes: [...new Set((Array.isArray(row?.hs_codes) ? row.hs_codes : []).map(normalizeHsCode).filter(Boolean))],
        product_names: [...new Set((Array.isArray(row?.product_names) ? row.product_names : []).map((value) => String(value || '').trim()).filter(Boolean))],
        source_id: SOURCE_ID,
