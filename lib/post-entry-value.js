...
(function (global) {
    ...
    function buildSpecialProgramDutyAdjustment(valueResult, rule, context = {}, addOnDuty = 0) {
        ...
        if (match.annex === 'II') {
            ...
            const ratePer100Kg = Number(context.specificDutyRatePer100Kg);
            ...
            return {
                ...
                summary: calculationComplete
                    ? `Annex II CN ${match.cnCode} suspends the ad valorem component. The retained TARIC specific duty is ${calculatedSpecificDuty.toFixed(2)} in the selected currency, based on the supplied entry-price/weight evidence.`
                    : `Annex II CN ${match.cnCode} suspends the ad valorem component, but the entry-price specific duty remains. The final adjusted duty cannot be completed until that specific component is obtained from TARIC.`
            };
        }
        if (match.annex === 'III') {
            ...
            return {
                ...
                summary: quotaConfirmed
                    ? `Annex III CN ${match.cnCode} uses quota ${match.orderNumber}. The official QUOTA database reports available balance; allocation remains first-come, first-served and must still be confirmed for the declaration.`
