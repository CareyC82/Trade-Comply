...
(function (global) {
    ...
    function resolveEuUsSpecialProgramApi() {
        if (global?.TraceWizeEuUsSpecialProgram) return global.TraceWizeEuUsSpecialProgram;
        if (typeof require === 'function') {
            try {
                return require('./eu-us-special-program');
            } catch (error) {
                return null;
            }
        }
    ...
    const euUsSpecialProgramApi = resolveEuUsSpecialProgramApi();
    ...
    function normalizeExactCodeOverride(raw = {}) {
        return {
            ...
            effectiveFrom: raw.effectiveFrom || raw.effective_from || '',
            effectiveTo: raw.effectiveTo || raw.effective_to || '',
            swsRate: raw.swsRate ?? raw.sws_rate ?? null,
            igstRate: raw.igstRate ?? raw.igst_rate ?? null,
            preferenceRateText: raw.preferenceRateText || raw.preference_rate_text || '',
            preferenceRateStatus: raw.preferenceRateStatus || raw.preference_rate_status || ''
        };
    }
    ...
    function buildDutySourceBreakdown(rule, addOnLayers = []) {
        ...
        if (rule.swsRate !== null && rule.swsRate !== undefined) rows.push({
            component: 'sws', label: 'Social Welfare Surcharge (SWS)', status: 'official_source_checked',
            source: baseSource, detail: `${(Number(rule.swsRate) * 100).toFixed(2)}% shown separately from BCD`,
            rate: Number(rule.swsRate), amount: null, hts: rule.sourceHts || '', url: rule.sourceUrl || '', lastCheckedAt: rule.lastCheckedAt || ''
        });
        if (rule.igstRate !== null && rule.igstRate !== undefined) rows.push({
            component: 'igst', label: 'Integrated GST (IGST)', status: 'official_source_checked',
            source: baseSource, detail: `${(Number(rule.igstRate) * 100).toFixed(2)}% import-tax layer; not merged into base customs duty`,
            rate: Number(rule.igstRate), amount: null, hts: rule.sourceHts || '', url: rule.sourceUrl || '', lastCheckedAt: rule.lastCheckedAt || ''
        });
        if (rule.preferenceRateText) rows.push({
            component: 'preference', label: 'FTA preferential rate', status: 'scope_check_required',
            source: baseSource, detail: `${rule.preferenceRateText} published preference; origin eligibility and supporting evidence must be confirmed before use`,
            rate: null, amount: null, hts: rule.sourceHts || '', url: rule.sourceUrl || '', lastCheckedAt: rule.lastCheckedAt || ''
        });
    ...
    function buildFilingGradeFocus(rule, context = {}) {
        ...
        if (hsCode.startsWith('850440')) {
            return {
                focus: 'Before correction, confirm the exact 850440 power-conversion line, charger versus power-supply function, Section 301 scope/exclusion status, and safety/energy evidence.',
                checklist: [
                    'Confirm whether the product is an EV charger, AC/DC adapter, inverter, converter, or embedded power supply.',
                    'Check the exact 10-digit HTS line, Chapter 99 / Section 301 treatment, and any active exclusion.',
                    'Keep UL/NRTL, energy, FCC/radio module if applicable, wiring, and product-function evidence with the entry file.'
                ]
            };
        }
        ...
        if (hsCode.startsWith('847130')) {
            return {
                focus: 'Before correction, confirm the portable ADP/tablet classification, cellular or Wi-Fi module scope, Section 301 treatment, battery evidence, and product-function support.',
                checklist: [
                    'Confirm exact 10-digit HTS line for tablet / portable ADP equipment versus phone, monitor, or accessory classification.',
                    'Check Chapter 99 / Section 301 treatment and any active exclusion tied to the exact tariff line.',
                    'Keep wireless/FCC, battery, processor, screen, and intended-use evidence with the entry file.'
                ]
            };
        }

        if (hsCode.startsWith('8542')) {
            return {
                focus: 'Before correction, confirm the exact integrated-circuit line, country of origin, Chapter 99/add-on scope, controlled-technology exposure, and end-use evidence.',
                checklist: [
                    'Confirm exact 10-digit HTS line for processor, memory, accelerator, controller, or other integrated circuit.',
                    'Check Chapter 99 / Section 301 treatment, exclusion status, and any end-use or controlled-technology red flags.',
                    'Keep die/package origin, technical specifications, performance data, and consignee/end-use evidence with the entry file.'
                ]
            };
        }
    ...
    function findExactCodeOverride(rule, hsCode, entryDate = '') {
        const normalized = normalizeHsCode(hsCode);
        if (!normalized || !Array.isArray(rule?.exactCodeOverrides)) {
            return null;
        }
        const matches = rule.exactCodeOverrides
            .filter((override) => {
                if (!override.hsCode) return false;
                const exactNationalLine = override.hsCode.length >= 8;
                if (exactNationalLine) {
                    if (normalized.length >= override.hsCode.length) {
                        if (normalized !== override.hsCode) return false;
                    } else if (normalized.length === 8) {
                        if (!override.hsCode.startsWith(normalized)) return false;
                    } else {
                        return false;
                    }
                } else if (!normalized.startsWith(override.hsCode)) {
                    return false;
                }
                if (entryDate && override.effectiveFrom && entryDate < override.effectiveFrom) return false;
                if (entryDate && override.effectiveTo && entryDate > override.effectiveTo) return false;
                return true;
            })
            .sort((a, b) => b.hsCode.length - a.hsCode.length);
        if (!matches.length) return null;
        if (normalized.length === 8 && matches.some((row) => row.hsCode.length > 8)) {
            const rates = new Set(matches.map((row) => row.baseRate));
            if (rates.size !== 1) return null;
            return {
                ...matches[0],
                hsCode: normalized,
                sourceHts: `${normalized} (all matched 10-digit TARIC lines share one rate)`
            };
        }
        return matches[0];
    }
    ...
    function applyExactCodeOverride(rule, override) {
        ...
        return {
            ...
            swsRate: override.swsRate,
            igstRate: override.igstRate,
            preferenceRateText: override.preferenceRateText,
            preferenceRateStatus: override.preferenceRateStatus
        };
    }
    ...
    function loadDutyRules() {
        ...
        if (isNodeRuntime()) {
            try {
                ...
                cachedSpecialPrograms = Array.isArray(payload.special_programs) ? payload.special_programs : [];
    ...
    function setDutyRulesForTest(rules, specialPrograms = null) {
        cachedDutyRules = Array.isArray(rules) ? rules.map(normalizeDutyRule) : null;
        if (Array.isArray(specialPrograms)) cachedSpecialPrograms = specialPrograms;
    }

    function setDutyRateData(payload = {}) {
        setDutyRulesForTest(payload.rules, payload.special_programs);
    }
    ...
    function findDutyRule(context = {}) {
        ...
        return rule ? applyExactCodeOverride(rule, findExactCodeOverride(rule, hsCode, context.entryDate)) : null;
    }
    ...
    function calculateDutyImpact(valueResult, context = {}, input = {}) {
        ...
        if (!rule) {
            const specialProgramAdjustment = buildSpecialProgramDutyAdjustment(valueResult, null, context, 0);
            const specialProgramCovered = Boolean(
                specialProgramAdjustment?.calculationComplete
                && specialProgramAdjustment.adjustedEstimatedDuty !== null
            );
            const estimatedDuty = specialProgramCovered
                ? specialProgramAdjustment.adjustedEstimatedDuty
                : null;
            const dutyVariance = estimatedDuty === null ? null : estimatedDuty - declaredDuty;
            const sourceBreakdown = buildDutySourceBreakdown(null);
            if (specialProgramAdjustment) {
                sourceBreakdown.push({
                    component: 'special_tariff_program',
                    label: specialProgramAdjustment.label,
                    status: 'conditional_program',
                    source: specialProgramAdjustment.legalBasis,
                    detail: specialProgramAdjustment.summary,
                    rate: specialProgramAdjustment.adjustedBaseRate,
                    amount: specialProgramAdjustment.adjustedEstimatedDuty,
                    hts: specialProgramAdjustment.cnCode,
                    url: specialProgramAdjustment.sourceUrl,
                    lastCheckedAt: specialProgramAdjustment.lastVerified
                });
            }
            return {
                covered: specialProgramCovered,
                declaredDuty,
                estimatedDuty,
                dutyVariance,
                totalRate: specialProgramCovered ? specialProgramAdjustment.adjustedBaseRate : null,
                rateLabel: specialProgramCovered ? specialProgramAdjustment.label : 'Rate not covered',
                tradeRemedy: specialProgramCovered
                    ? specialProgramAdjustment.summary
                    : 'No built-in duty signal for this route / HS prefix yet.',
                conclusion: specialProgramCovered
                    ? `Conditional special-program duty is ${estimatedDuty.toFixed(2)} in entered currency; retain the standard treatment until all eligibility evidence is complete.`
                    : 'Duty impact cannot be estimated from the current local rate table.',
                action: specialProgramCovered
                    ? 'Use the conditional amount only after the official Annex scope and required evidence are confirmed.'
                    : 'Use the customs value result, then confirm the exact tariff line in the destination tariff database.',
                sourceBreakdown,
        ...
        const specialProgramAdjustment = buildSpecialProgramDutyAdjustment(
            valueResult,
            rule,
            context,
            addOnDuty
        ...
        const sourceBreakdown = buildDutySourceBreakdown(rule, addOnLayers);
        if (specialProgramAdjustment) {
            sourceBreakdown.push({
                component: 'special_tariff_program',
                label: specialProgramAdjustment.label,
    ...
    function buildSpecialProgramDutyAdjustment(valueResult, rule, context = {}, addOnDuty = 0) {
        const gate = buildOriginEvidenceGate(context);
        if (!gate || (!gate.annexMatched && gate.scopeStatus === 'official_annex_confirmation_required')) return null;
        const hasStandardRate = Number.isFinite(Number(rule?.baseRate));
        const standardBaseRate = hasStandardRate ? Number(rule.baseRate) : null;
        const standardBaseDuty = standardBaseRate === null ? null : valueResult.customsValue * standardBaseRate;
        const standardEstimatedDuty = standardBaseDuty === null ? null : standardBaseDuty + addOnDuty;
        const match = (gate.annexMatches || [])
            .slice()
            .sort((left, right) => String(right.normalizedCode || '').length - String(left.normalizedCode || '').length)[0] || null;
        const base = {
            id: 'EU-US-2026-1455',
            label: 'EU-US adjusted-duty program',
            legalBasis: 'Regulation (EU) 2026/1455',
            sourceUrl: gate.sourceUrl,
            lastVerified: gate.lastVerified || '',
            cnCode: gate.hsCode,
            annex: match?.annex || '',
            annexCnCode: match?.cnCode || '',
            standardBaseRate,
            standardBaseDuty,
            standardEstimatedDuty,
            adjustedBaseRate: null,
            adjustedBaseDuty: null,
            adjustedEstimatedDuty: null,
            potentialSavings: null,
            calculationComplete: false,
            evidenceRequired: true,
            scopeStatus: gate.scopeStatus,
            eligibility: gate.eligibility,
            declarationCodes: gate.declarationCodes,
            evidence: gate.evidence,
            summary: gate.summary
        };
        if (!gate.effectiveOnEntryDate || !gate.annexMatched || !gate.treatmentConfirmed || !match) {
            return base;
        }
        if (match.annex === 'I') {
            const calculationComplete = standardBaseDuty !== null;
            return {
                ...base,
                adjustedBaseRate: 0,
                adjustedBaseDuty: 0,
                adjustedEstimatedDuty: addOnDuty,
                potentialSavings: standardBaseDuty,
                calculationComplete,
                summary: calculationComplete
                    ? `Annex I CN ${match.cnCode} conditionally reduces the Common Customs Tariff base duty from ${(standardBaseRate * 100).toFixed(2)}% to 0%. Article 59a origin and transport evidence must be complete before filing at the adjusted rate.`
                    : `Annex I CN ${match.cnCode} conditionally sets the Common Customs Tariff duty to 0%, but this route has no maintained standard-rate comparison yet. Article 59a evidence is still required.`
            };
        }
        if (match.annex === 'II') {
            const suspendedRate = match.suspendedAdValoremRate !== null
                && match.suspendedAdValoremRate !== ''
                && Number.isFinite(Number(match.suspendedAdValoremRate))
                ? Number(match.suspendedAdValoremRate)
                : standardBaseRate;
            const suspendedAmount = suspendedRate === null ? null : valueResult.customsValue * suspendedRate;
            const hasExplicitSpecificDuty = context.specificDutyAmount !== null
                && context.specificDutyAmount !== undefined
                && context.specificDutyAmount !== '';
            const explicitSpecificDuty = Number(context.specificDutyAmount);
            const automaticSpecificDuty = gate.autoSpecificDuty || null;
            const hasAutomaticZeroDuty = automaticSpecificDuty?.unit === 'NONE' && automaticSpecificDuty.amount === 0;
            const ratePer100Kg = context.specificDutyRatePer100Kg !== null
                && context.specificDutyRatePer100Kg !== undefined
                && context.specificDutyRatePer100Kg !== ''
                ? Number(context.specificDutyRatePer100Kg)
                : Number(automaticSpecificDuty?.rate_per_100kg);
            const netWeightKg = Number(context.netWeightKg);
            const calculatedSpecificDuty = hasExplicitSpecificDuty && Number.isFinite(explicitSpecificDuty) && explicitSpecificDuty >= 0
                ? explicitSpecificDuty
                : hasAutomaticZeroDuty
                    ? 0
                : Number.isFinite(ratePer100Kg) && ratePer100Kg >= 0 && Number.isFinite(netWeightKg) && netWeightKg > 0
                    ? (netWeightKg / 100) * ratePer100Kg
                    : null;
            const automaticSourceUsed = !hasExplicitSpecificDuty
                && (hasAutomaticZeroDuty || (automaticSpecificDuty?.rate_per_100kg !== null && automaticSpecificDuty?.rate_per_100kg !== undefined));
            const calculationComplete = calculatedSpecificDuty !== null && suspendedAmount !== null;
            return {
                ...base,
                standardBaseRate: suspendedRate,
                standardBaseDuty: suspendedAmount,
                standardEstimatedDuty: suspendedAmount === null ? null : suspendedAmount + addOnDuty,
                adjustedBaseRate: 0,
                adjustedBaseDuty: calculatedSpecificDuty,
                adjustedEstimatedDuty: calculatedSpecificDuty === null ? null : calculatedSpecificDuty + addOnDuty,
                potentialSavings: suspendedAmount,
                calculationComplete,
                specificDutyAmount: calculatedSpecificDuty,
                specificDutySource: automaticSourceUsed ? 'Official TARIC automatic formula' : 'User-confirmed TARIC amount',
                specificDutyRows: gate.specificDutyRows || [],
                summary: calculationComplete
                    ? `Annex II CN ${match.cnCode} suspends the ad valorem component. The retained TARIC specific duty is ${calculatedSpecificDuty.toFixed(2)} in the selected currency, based on ${automaticSourceUsed ? 'the synchronized official TARIC formula' : 'the supplied entry-price/weight evidence'}.`
                    : `Annex II CN ${match.cnCode} suspends the ad valorem component, but the entry-price specific duty remains conditional. TARIC returned ${gate.specificDutyRows?.length || 0} applicable formula row(s); enter the declared entry price and official specific component when no single formula can be applied automatically.`
            };
        }
        if (match.annex === 'III') {
            const quotaRate = match.inQuotaAdValoremRate !== null
                && match.inQuotaAdValoremRate !== ''
                && Number.isFinite(Number(match.inQuotaAdValoremRate))
                ? Number(match.inQuotaAdValoremRate)
                : null;
            const quotaConfirmed = context.quotaAvailable === true || gate.quotaBalanceAvailable === true;
            const adjustedBaseDuty = quotaConfirmed && quotaRate !== null
                ? valueResult.customsValue * quotaRate
                : null;
            return {
                ...base,
                adjustedBaseRate: quotaRate,
                adjustedBaseDuty,
                adjustedEstimatedDuty: adjustedBaseDuty === null ? null : adjustedBaseDuty + addOnDuty,
                potentialSavings: adjustedBaseDuty === null || standardBaseDuty === null
                    ? null
                    : standardBaseDuty - adjustedBaseDuty,
                calculationComplete: quotaConfirmed && quotaRate !== null && standardBaseDuty !== null,
                quotaStatus: gate.quotaRows || [],
                quotaAlert: gate.quotaAlert || 'unknown',
                summary: quotaConfirmed
                    ? `Annex III CN ${match.cnCode} uses quota ${match.orderNumber}. Official quota status: ${gate.quotaAlert || 'available'}. Allocation remains first-come, first-served and must still be confirmed for the declaration.`
                    : `Annex III CN ${match.cnCode} uses quota ${match.orderNumber}. The in-quota rate can be applied only after live quota availability and allocation are confirmed in TARIC.`
            };
        }
        return base;
    }
    ...
    function buildReviewChecklist(context = {}) {
        ...
        const originGate = buildOriginEvidenceGate(context);
        if (originGate) {
            checklist.push(...originGate.checklist);
        }
        return checklist;
    }

    function buildOriginEvidenceGate(context = {}) {
        const importCountry = String(context.importCountryCode || '').toUpperCase();
        const originCountry = String(context.originCountryCode || '').toUpperCase();
        const hsCode = String(context.hsCode || '').trim() || 'the declared CN code';
        if (!['EU', 'DE', 'NL'].includes(importCountry) || originCountry !== 'US') {
            return null;
        }
        loadDutyRules();
        const resolution = euUsSpecialProgramApi?.resolveProgramTreatment
            ? euUsSpecialProgramApi.resolveProgramTreatment({
                programs: cachedSpecialPrograms,
                importCountry,
                originCountry,
                hsCode,
                entryDate: context.entryDate,
                descriptionConfirmed: context.descriptionConfirmed,
                originEvidenceConfirmed: context.originEvidenceConfirmed,
                transportEvidenceConfirmed: context.transportEvidenceConfirmed,
                declarationCodesConfirmed: context.declarationCodesConfirmed,
                quotaAvailable: context.quotaAvailable,
                specificDutyConfirmed: context.specificDutyConfirmed
            })
            : null;
        const matchedTreatment = resolution?.matched
            ? resolution.treatmentLabel
            : '';
        const scopeStatus = resolution?.scopeStatus || 'official_annex_confirmation_required';
        const scopeSummary = resolution && !resolution.effectiveOnEntryDate
            ? `Regulation (EU) 2026/1455 applies from ${resolution.effectiveFrom}; it is not applied to entry date ${resolution.entryDate || context.entryDate}.`
            : resolution?.matched
            ? resolution.requiresDescriptionConfirmation
                ? `${hsCode} matches an ex CN entry, so the product description must also satisfy the official Annex wording before treatment is claimed.`
                : `${hsCode} is covered by ${resolution.matches.map((row) => `Annex ${row.annex} CN ${row.cnCode}`).join(', ')}. ${matchedTreatment}`
            : `No Regulation (EU) 2026/1455 Annex match was found for ${hsCode}; do not claim the adjusted treatment.`;
        return {
            title: 'EU non-preferential origin evidence gate',
            hsCode,
            effectiveFrom: '2026-07-01',
            scopeStatus,
            eligibility: resolution?.eligibility || null,
            annexConfirmationRequired: !resolution?.eligible,
            annexMatched: Boolean(resolution?.matched),
            treatmentConfirmed: Boolean(resolution?.eligible),
            effectiveOnEntryDate: resolution?.effectiveOnEntryDate !== false,
            annexMatches: resolution?.matches || [],
            quotaRows: resolution?.quotaRows || [],
            quotaBalanceAvailable: Boolean(resolution?.quotaBalanceAvailable),
            quotaLastChecked: resolution?.quotaLastChecked || '',
            summary: `${scopeSummary} Article 59a also requires direct-transport or customs-supervision/non-alteration evidence for US-origin goods.`,
            treatments: [
                'Annex I: 0% Common Customs Tariff duty for listed CN lines.',
                'Annex II: ad valorem component removed; the specific entry-price duty remains.',
                'Annex III: reduced in-quota rates within the applicable 12-month quota period.'
            ],
            declarationCodes: {
                measureTypes: ['142', '145'],
                preferenceCode: '300',
                supportingDocument: 'U190'
            },
            checklist: [
                `Confirm that ${hsCode} is expressly listed in Annex I, II, or III of Regulation (EU) 2026/1455; do not infer eligibility from a broad HS heading.`,
                'Apply the matching Annex treatment only: Annex I 0%, Annex II removal of the ad valorem component, or Annex III in-quota rate.',
                'Validate TARIC measure type 142/145, preference code 300, and supporting document U190 where required.',
                'Keep direct-transport evidence, or prove customs supervision and non-alteration when goods moved, stored, or were split through another country.',
                'Retain the non-preferential proof of origin and any ELAN electronic certificate used for a special non-preferential import arrangement.'
            ],
            evidence: [
                'Official Annex line confirming the declared CN code and applicable treatment.',
                'TARIC declaration support for measure type 142/145, preference code 300, and document U190 where applicable.',
                'Non-preferential proof of origin supporting the United States origin declaration.',
                'Through bill of lading or equivalent direct-transport record.',
                'For third-country routing: customs-supervision, storage/splitting, seal, handling, and non-alteration records.',
                'ELAN electronic certificate of origin where the special non-preferential import arrangement requires one.'
            ],
            lastVerified: resolution?.program?.last_verified_at || '',
            sourceUrl: 'https://eur-lex.europa.eu/legal-content/EN/TXT/?uri=CELEX:32026R1455',
            originProcedureUrl: 'https://eur-lex.europa.eu/legal-content/EN/TXT/?uri=CELEX:32026R1422'
        };
    }
    ...
    function buildEvidenceList(context = {}) {
        ...
        const originGate = buildOriginEvidenceGate(context);
        if (originGate) {
            evidence.push(...originGate.evidence);
        }
        return evidence;
    }
