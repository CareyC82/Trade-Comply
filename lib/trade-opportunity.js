...
function buildCommercialOpportunity({ market, productSignal, dutySignal, coverage, profile }) {
    ...
    const quickDecision = !readyForPricing
        ...
        : commercialRule.complianceFriction === 'Very high'
            ? `Control-led opportunity: ${countryLabel(market)} has demand, but sanctions, end-use, or controlled-technology checks come first.`
        : rateValue === 0
    ...
    const salesAngle = `${commercialRule.demandStrength} demand; ${categoryHooks[productSignal.id] || categoryHooks.general}; lead with ${profile.advantage.toLowerCase()}.`;
