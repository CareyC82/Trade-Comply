...
function resolveEuUsSpecialProgramApi() {
    if (typeof globalThis !== 'undefined' && globalThis.TraceWizeEuUsSpecialProgram) {
        return globalThis.TraceWizeEuUsSpecialProgram;
    }
    if (typeof require === 'function') {
        try {
...
function buildEuOriginEvidenceGate({ origin, destination, via = '', hsCode = '', dutyRates = {} } = {}) {
    const normalizedOrigin = normalizeCountry(origin);
    const normalizedDestination = normalizeCountry(destination);
    if (normalizedOrigin !== 'US' || !['EU', 'DE', 'NL'].includes(normalizedDestination)) {
        return null;
    }
    const destinationLabel = countryLabel(normalizedDestination);
    const viaLabel = via ? countryLabel(via) : '';
    const declaredCode = String(hsCode || '').trim() || 'the declared CN code';
    const resolution = euUsSpecialProgramApi?.resolveProgramTreatment
        ? euUsSpecialProgramApi.resolveProgramTreatment({
            ...
            importCountry: normalizedDestination,
    ...
    const scopeSummary = resolution?.matched
        ? resolution.requiresDescriptionConfirmation
            ? `${declaredCode} matches an ex CN entry; confirm that the product description also matches the Annex wording.`
            : `${declaredCode} is covered by ${resolution.matches.map((row) => `Annex ${row.annex} CN ${row.cnCode}`).join(', ')}. ${resolution.treatmentLabel}`
        : `No Regulation (EU) 2026/1455 Annex match was found for ${declaredCode}; do not include adjusted-duty savings in this route.`;
    return {
        label: 'EU Article 59a origin-evidence gate',
        hsCode: declaredCode,
        effectiveFrom: '2026-07-01',
        scopeStatus: resolution?.scopeStatus || 'official_annex_confirmation_required',
        ...
        summary: `${scopeSummary} Article 59a evidence must also prove direct transport or customs supervision/non-alteration through any third country entering ${destinationLabel}.`,
        treatments: [
        ...
        action: viaLabel
            ? `First confirm the exact Annex CN line, then keep ${viaLabel} customs-supervision, storage/splitting, seal, handling, and non-alteration records; transit does not remove the US-origin proof requirement.`
            : 'Confirm the exact Annex CN line and TARIC declaration codes, then keep non-preferential origin and direct-transport evidence with the quote and import file.',
...
function buildMarketCard({ market, from, to, focus, productSignal, dutyRates, priorityMatrix, ruleTags }) {
    ...
    const originEvidenceGate = buildEuOriginEvidenceGate({
        origin: from,
        destination: market,
        hsCode: priorityRoute?.hs_code || dutySignal?.matchedHsCode || '',
        dutyRates
    });
    ...
    if (originEvidenceGate) {
        rejectionReasons.push(`${originEvidenceGate.label} must be documented when adjusted US-origin duties or tariff quotas are claimed.`);
    }
    if (commercialOpportunity.landedCostRisk === 'High') {
    ...
    const baseRouteDecisionSummary = exportControlGate?.severity === 'Critical'
    ...
    const routeDecisionSummary = originEvidenceGate
        ? `${baseRouteDecisionSummary} ${originEvidenceGate.label} applies when Regulation (EU) 2026/1455 treatment is claimed.`
        : baseRouteDecisionSummary;
    ...
    const sourceEvidence = [
        ...
        ...(originEvidenceGate ? [{
            label: 'Origin evidence gate',
            detail: `${originEvidenceGate.summary} ${originEvidenceGate.action} Source: ${originEvidenceGate.sourceUrl}`
        }] : []),
        {
            label: 'Green / ESG',
...
function buildTransitComparison({ card, from, to, productSignal, dutyRates, priorityMatrix, directMarket }) {
    ...
    const euOriginEvidenceGate = buildEuOriginEvidenceGate({
        origin: from,
        destination: to,
        via,
        hsCode: secondPriorityRoute?.hs_code || card.hsCode || '',
        dutyRates
    });
    const costScopeNote = 'Duty/tax signal only; excludes logistics, lead time, financing, inventory, broker, and operational costs.';
    const costConclusion = !Number.isFinite(combinedRate)
        ? 'Combined cost is not covered yet.'
        : Number.isFinite(delta) && delta <= -0.02
        ? `Potential duty/tax signal advantage versus direct route: ${rateLabel(Math.abs(delta))} lower maintained duty/tax signal before logistics and filing costs. ${originSavingsCaveat}`
        : Number.isFinite(delta) && delta >= 0.02
        ? `No maintained duty/tax advantage versus direct route: ${rateLabel(delta)} higher duty/tax signal.`
    ...
    const transitLegalGate = {
        ...
        document_evidence_required: [
            ...
            ...(euOriginEvidenceGate ? [
                'Official Regulation (EU) 2026/1455 Annex line for the declared CN code',
                'TARIC measure type 142/145, preference code 300, and U190 support where applicable',
                'EU Article 59a direct-transport or customs-supervision/non-alteration evidence'
            ] : [])
        ],
    ...
    return {
        ...
        sourceEvidence: [
            ...
            },
            {
                ...
                detail: `${originNote} ${transitAuthenticityDecision}${euOriginEvidenceGate ? ` ${euOriginEvidenceGate.summary} ${euOriginEvidenceGate.action}` : ''}`
            },
            {
                label: 'Control gate',
                detail: controlGate
        ...
        transitWarning: `${costConclusion} Combined duty/tax signal: ${combinedLabel} (${combinedCostLabel}) vs direct ${directLabel} (${directCostLabel}); delta: ${deltaRateLabel} (${deltaCostLabel}) per $1k. First leg ${fromLabel} -> ${viaLabel}: ${firstLabel}; second leg ${viaLabel} -> ${toLabel}: ${secondLabel}. ${costScopeNote} ${secondLegScopeNote} ${originSavingsCaveat} ${originNote}${euOriginEvidenceGate ? ` ${euOriginEvidenceGate.summary}` : ''}`,
        routeKind: 'transit',
        transitCostStatus,
        transitReason,
        originSavingsCaveat,
        originEvidenceGate: euOriginEvidenceGate,
        transitScore,
        transitComparison: {
            via,
            viaLabel,
