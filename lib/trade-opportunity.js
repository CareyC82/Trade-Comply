...
function directRouteVerdict({ exportControlGate, benchmarkOnly, dutySignal, commercialOpportunity }) {
    if (exportControlGate?.severity === 'Critical') {
        return {
            label: 'Control first',
            tone: 'critical',
            action: 'Clear export-control, end-use, and restricted-party review before route optimization.'
        };
    }
...
function transitRouteVerdict({ transitCostStatus, delta, controlGate, viaLabel }) {
    if (controlGate?.severity === 'Critical') {
        return {
            label: 'Control first',
            tone: 'critical',
            action: `${viaLabel} cannot be used as a workaround for export-control or end-use restrictions.`
        };
    }
...
function buildBusinessDecisionSummary({ selectedMarket, transitRoutes, best, productSignal }) {
    ...
    return {
        ...
        primaryAction: controlFirst
            ? 'Clear export-control, end-use, restricted-party, and re-export evidence before using any route recommendation.'
