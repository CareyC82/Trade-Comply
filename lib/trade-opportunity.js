...
const PRODUCT_SIGNALS = [
    ...
    {
        id: 'semiconductor',
        match: /(?!(?:.*\b(?:ai\s*server|gpu\s*server|edge\s*ai|data\s*center|server\s*rack|storage\s*server)\b))(chip|semiconductor|gpu\s*(?:chip|accelerator)?|accelerator\s*(?:chip|card)?|processor|\bic\b|integrated circuit|芯片|半导体)/i,
    ...
    },
    {
        ...
        match: /(router|wi[-\s]?fi|network\s*(?:switch|gateway)|ethernet\s*switch|access\s*point|telecom\s*equipment|base\s*station|路由器|交换机|网关)/i,
...
function detectProductSignal(product = '') {
    ...
    if (matched?.id === 'semiconductor' && /\b(gpu|gpgpu|ai\s*accelerator|accelerator\s*(?:chip|card)?)\b/i.test(text)) {
        return {
            ...matched,
    ...
    if (matched?.id === 'smartphone' && /(laptop|computer|电脑)/i.test(text)) {
        return {
            ...matched,
...
function findDutySignal({ from, to, productSignal, dutyRates, hsCode = '' }) {
...
function buildMarketCard({ market, from, to, focus, productSignal, dutyRates, priorityMatrix }) {
    ...
    const opportunitySignal = buildOpportunitySignal({ cardLabel: countryLabel(market), market, tag, dutySignal, coverage, productSignal, profile, readiness });
    return {
        market,
...
function buildOpportunityInsights(input = {}) {
    ...
    const compared = [
        selectedMarket,
        ...cards.filter((card) => card.market !== to).slice(0, 8)
    ].sort((a, b) => b.score - a.score || b.readinessRank - a.readinessRank || a.label.localeCompare(b.label));
