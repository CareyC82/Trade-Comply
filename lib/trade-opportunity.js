...
function buildTransitComparison({ card, from, to, productSignal, dutyRates, priorityMatrix, directMarket }) {
    ...
    return {
        ...
        sourceEvidence: [
            ...
            {
                label: 'Combined cost',
                detail: `${fromLabel} -> ${viaLabel}: ${firstLabel}; ${viaLabel} -> ${toLabel}: ${secondLabel}; total ${combinedLabel}; delta ${Number.isFinite(delta) ? `${delta >= 0 ? '+' : '-'}${rateLabel(Math.abs(delta))}` : 'not covered'} vs direct.`
            },
            {
                label: 'Source coverage',
                detail: `First leg: ${card.coverageLabel}; second leg: ${secondCoverage.label}.`
            },
            {
