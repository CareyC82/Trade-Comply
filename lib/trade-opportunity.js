...
function detectProductSignal(product = '') {
    ...
    if (matched?.id === 'semiconductor' && /\b(gpu|gpgpu|ai\s*accelerator|accelerator\s*(?:chip|card)?|h100|h200|h800|a100|a800|b100|b200|l40s?|mi300x?|gb200|rtx\s*pro)\b/i.test(text)) {
        return {
            ...matched,
...
function findDutySignal({ from, to, productSignal, dutyRates, hsCode = '', strictOrigin = false }) {
    ...
    const destination = exact || wildcard || (!strictOrigin && rules.find((rule) => (
        normalizeCountry(rule.import_country) === to
        && hsMatches(rule, productSignal.hsPrefixes)
    )));
    if (!destination) {
...
function buildCommercialOpportunity({ market, productSignal, dutySignal, coverage, profile }) {
    ...
    const readyForPricing = coverage.sourceTrust === 'official_exact_rate'
...
function exportControlProfile(productSignal) {
    const profiles = {
        ...
        ai_compute: {
            ...
            summary: 'AI servers, GPU servers, edge AI computers, and accelerator systems may include controlled GPUs, high-performance computing functions, encryption, software, or restricted end-use exposure.',
            checks: [
                ...
                'Confirm whether the shipment includes controlled hardware, software, technology, service support, or re-export content.'
            ]
        },
        data_center: {
...
function buildExportControlGate({ from, to, market, productSignal }) {
    ...
    const verySensitive = productSignal.id === 'semiconductor' || productSignal.id === 'ai_compute';
    const severity = restrictedDestination || (origin === 'US' && chinaDestination && verySensitive)
        ? 'Critical'
        : 'Review Required';
...
function buildMarketCard({ market, from, to, focus, productSignal, dutyRates, priorityMatrix }) {
    ...
    return {
        ...
        opportunitySignal,
        commercialDecision: commercialOpportunity.quickDecision,
        exportControlGate,
...
function buildTransitComparison({ card, from, to, productSignal, dutyRates, priorityMatrix, directMarket }) {
    ...
    const costConclusion = !Number.isFinite(combinedRate)
        ...
        ? `Potential cost advantage versus direct route: ${rateLabel(Math.abs(delta))} lower duty/tax signal.`
        : Number.isFinite(delta) && delta >= 0.02
    ...
    return {
        ...
        commercialDecision: transitDecision.headline,
        businessAction: transitDecision.reason,
        routeScopeLabel: `Transit comparison: ${fromLabel} -> ${viaLabel} -> ${toLabel}`,
        transitWarning: `${costConclusion} First leg ${fromLabel} -> ${viaLabel}: ${firstLabel}; second leg ${viaLabel} -> ${toLabel}: ${secondLabel}; combined signal: ${combinedLabel}. ${originNote}`,
        transitScore,
        transitComparison: {
            via,
            viaLabel,
            ...
            directRate: Number.isFinite(directRate) ? rateLabel(directRate) : 'not covered',
            deltaRate: Number.isFinite(delta) ? `${delta >= 0 ? '+' : '-'}${rateLabel(Math.abs(delta))}` : 'not covered',
            costConclusion,
            originNote,
            decision: transitDecision,
            secondCoverageLabel: secondCoverage.label,
...
function classifyOpportunityWorkbenchBucket({ selected = {}, bestIsSelected = true, priorityScore = 0 }) {
    ...
    const officialEnough = ['official_exact_rate', 'official_duty_tax_estimate', 'mixed_official_estimate'].includes(sourceTrust);
...
function buildOpportunityPriorityList({
    ...
    routes.forEach((route) => {
        ...
        const priorityScore = Math.max(0, Math.min(100, Math.round(
            (bestScore * 0.55)
            + (selectedScore * 0.35)
            + (selected.sourceTrust === 'official_exact_rate' ? 8 : selected.sourceTrust === 'official_duty_tax_estimate' ? 6 : selected.sourceTrust === 'mixed_official_estimate' ? 4 : 0)
