...
function buildEuOriginEvidenceGate({ origin, destination, via = '' } = {}) {
    ...
    return {
        ...
        summary: `If Regulation (EU) 2026/1455 adjusted duties or tariff quotas are claimed for US-origin goods entering ${destinationLabel}, the file must prove direct transport or customs supervision/non-alteration through any third country.`,
        action: viaLabel
            ? `Keep ${viaLabel} customs-supervision, storage/splitting, seal, handling, and non-alteration records; transit does not remove the US-origin proof requirement.`
            : 'Keep non-preferential origin and direct-transport evidence with the quote and import file; use an ELAN electronic certificate where the special arrangement requires it.',
...
function buildMarketCard({ market, from, to, focus, productSignal, dutyRates, priorityMatrix, ruleTags }) {
    ...
    const originEvidenceGate = buildEuOriginEvidenceGate({ origin: from, destination: market });
...
function buildTransitComparison({ card, from, to, productSignal, dutyRates, priorityMatrix, directMarket }) {
    ...
    const euOriginEvidenceGate = buildEuOriginEvidenceGate({ origin: from, destination: to, via });
    ...
    const transitLegalGate = {
        ...
        document_evidence_required: [
            ...
            ...(euOriginEvidenceGate ? ['EU Article 59a direct-transport or customs-supervision/non-alteration evidence'] : [])
        ],
