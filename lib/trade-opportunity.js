...
function buildCommercialOpportunity({ market, productSignal, dutySignal, coverage, profile }) {
    ...
    const salesAngle = `${categoryHooks[productSignal.id] || categoryHooks.general}; lead with ${profile.advantage.toLowerCase()}.`;
    ...
    const landedCostRisk = rateValue === null
        ...
        : rateValue === 0
            ...
            : rateValue <= 0.1
                ? 'Medium'
                : 'High';
    const marketRole = market === 'SG' || market === 'NL'
        ? 'Regional distribution hub'
        : market === 'VN' || market === 'MY' || market === 'MX'
            ? 'Manufacturing / alternative supply-chain market'
            : market === 'US' || market === 'EU' || market === 'DE' || market === 'JP' || market === 'KR'
