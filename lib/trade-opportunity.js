...
function buildGreenComplianceSignal({ market, productSignal, ruleTags = [] }) {
    ...
    if (['solar', 'battery', 'ev_charger', 'smartphone', 'tablet', 'data_center', 'ai_compute', 'semiconductor', 'network_equipment', 'optical_module'].includes(productSignal.id)) {
        return {
            status: 'gap',
            readiness: 'ESG gap',
            scoreBoost: -2,
...
function buildTransitComparison({ card, from, to, productSignal, dutyRates, priorityMatrix, directMarket }) {
    ...
    const costConclusion = !Number.isFinite(combinedRate)
        ...
        ? `Potential duty-cost advantage versus direct route: ${rateLabel(Math.abs(delta))} lower duty/tax signal.`
        : Number.isFinite(delta) && delta >= 0.02
