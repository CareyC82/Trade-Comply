...
const { diskStatus } = require('./consumer-runtime');

const execFileAsync = promisify(execFile);
const MAX_FILE_BYTES = 10 * 1024 * 1024;
const MAX_FILES_PER_ACCOUNT = 20;
...
function executableStatus(command) {
    ...
    if (!value) return { command: '', available: false };
    ...
    return { command: value, available: Boolean(resolved), resolved: resolved || '' };
}
...
function readJson(file, fallback) {
    try { return JSON.parse(fs.readFileSync(file, 'utf8')); }
        ...
        throw new Error(`Consumer database is unreadable: ${error.message}`);
    }
}
...
function extractFields(text, expectedModel = '') {
    ...
    const expiryDate = take(/(?:expiry|expiration|valid\s*until|validity\s*date)\s*[:#]?\s*([0-9A-Za-z,./ -]{6,30})/i);
    const standards = [...new Set(compact.match(/(?:EN|IEC|ISO|FCC|ETSI|UL|ASTM|PSE|DENAN|IMDA)\s*[A-Z0-9-]*(?:\s*\d{2,6}(?:[-:.]\d+)*)?|UN\s*38\.3/gi) || [])].slice(0, 12);
    ...
    const adapterModel = take(/(?:adapter|adaptor|power\s*supply)\s*model(?:\s*(?:no|number))?\s*[:#]?\s*([A-Z0-9][A-Z0-9._/-]{2,40})/i);
    const radioModule = take(/(?:radio|wireless|wi-?fi|bluetooth)\s*module(?:\s*(?:model|no|number))?\s*[:#]?\s*([A-Z0-9][A-Z0-9._/-]{2,40})/i);
    const hardwareRevision = take(/(?:hardware|hw)\s*(?:revision|rev|version)\s*[:#]?\s*([A-Z0-9][A-Z0-9._/-]{0,40})/i);
    ...
    const typedMissingFields = documentKind === 'FCC' ? [
        !fccId && 'FCC ID',
        !standards.some((item) => /FCC/i.test(item)) && 'FCC rule / test standard'
    ] : documentKind === 'UN38.3' ? [
        !batteryModel && 'battery model',
        !standards.some((item) => /UN\s*38\.3/i.test(item)) && 'UN38.3 standard reference'
    ] : documentKind === 'CE / RED' ? [
        !standards.some((item) => /(?:ETSI|EN\s)/i.test(item)) && 'ETSI / EN radio standard'
    ] : documentKind === 'PSE' ? [
        !standards.some((item) => /(?:PSE|DENAN)/i.test(item)) && 'PSE / DENAN scope reference'
    ] : documentKind === 'IMDA' ? [
        !reportNumber && 'IMDA registration / certificate number',
        !standards.some((item) => /IMDA/i.test(item)) && 'IMDA technical specification'
    ] : [];
    ...
    return {
        model, manufacturer, reportNumber, reportDate, expiryDate, standards, fccId, batteryModel, adapterModel, radioModule, hardwareRevision, documentKind, missingFields,
...
class ConsumerService {
    constructor(options = {}) {
        ...
        this.databaseBackupFile = path.join(this.root, 'database.backup.json');
        this.databaseLockFile = path.join(this.root, 'database.lock');
        ...
        if (process.env.NODE_ENV === 'production' && (!this.explicitSessionSecret || !this.explicitFileKey)) {
            throw new Error('Production requires CONSUMER_SESSION_SECRET and CONSUMER_FILE_ENCRYPTION_KEY.');
        }
        this.fetch = options.fetch || globalThis.fetch;
        this.parseDocument = options.parseDocument || null;
        ...
        this.auditStorage({ repair: true });
    }

    read() { return readJson(this.databaseFile, { users: [], assessments: [], files: [] }); }
    write(db) {
        if (fs.existsSync(this.databaseFile)) {
            const current = readJson(this.databaseFile, null);
            if (current) atomicJson(this.databaseBackupFile, current);
        }
        atomicJson(this.databaseFile, db);
    }
    update(mutator) {
        let lock;
        for (let attempt = 0; attempt < 2 && lock === undefined; attempt += 1) {
            try {
                lock = fs.openSync(this.databaseLockFile, 'wx', 0o600);
            } catch (error) {
                if (error.code !== 'EEXIST') throw error;
                let age;
                try { age = Date.now() - fs.statSync(this.databaseLockFile).mtimeMs; }
                catch (statError) {
                    if (statError.code === 'ENOENT') continue;
                    throw statError;
                }
                if (age > 30000 && attempt === 0) {
                    try { fs.unlinkSync(this.databaseLockFile); }
                    catch (unlinkError) { if (unlinkError.code !== 'ENOENT') throw unlinkError; }
                    continue;
                }
                throw Object.assign(new Error('Consumer data is busy. Retry the request.'), { status: 503 });
            }
        }
        try {
            const db = this.read();
            ...
            try { fs.unlinkSync(this.databaseLockFile); } catch {}
        }
    }
    ...
    register(emailValue, password) {
        ...
        const user = this.update((db) => {
            if (db.users.some((item) => item.email === email)) throw Object.assign(new Error('This email is already registered.'), { status: 409 });
            const record = { id: crypto.randomUUID(), email, passwordHash: hashPassword(password), createdAt: new Date().toISOString() };
    ...
    deleteAssessment(userId, id) {
        return this.update((db) => {
            ...
            return before !== db.assessments.length;
        });
    }
    ...
    saveFile(userId, payload) {
        ...
        if (!raw || !/^[A-Za-z0-9+/]+={0,2}$/.test(raw) || raw.length % 4 !== 0) {
            throw Object.assign(new Error('File data is not valid Base64.'), { status: 400 });
        }
        ...
        const record = {
            ...
            expiresAt: new Date(this.now() + RETENTION_DAYS * 86400000).toISOString()
        };
        return this.update((db) => {
            const activeFiles = db.files.filter((item) => item.userId === userId).length;
            if (activeFiles >= MAX_FILES_PER_ACCOUNT) {
                throw Object.assign(new Error(`Delete an existing file before uploading more than ${MAX_FILES_PER_ACCOUNT} files.`), { status: 409 });
            }
            if (db.files.some((item) => item.userId === userId && item.sha256 === record.sha256)) {
                throw Object.assign(new Error('This exact file is already stored in your private workspace.'), { status: 409 });
            }
    ...
    async parseFile(userId, id) {
        ...
        try {
            if (this.parseDocument) {
                const output = await this.parseDocument({ buffer: plain, type: file.type, file: { ...file } });
                text = String(output?.text || ''); engine = output?.engine || 'test-parser';
            ...
            const message = unavailable
                ? 'Document verification service is unavailable. No file was approved; try again later.'
                : 'Document could not be read. No file was approved; upload a searchable PDF or a clear, upright image showing every page.';
            this.update((latestDb) => {
                const latestFile = latestDb.files.find((item) => item.id === id && item.userId === userId);
                if (latestFile) {
                    latestFile.status = unavailable ? 'parser_unavailable' : 'parse_failed';
                    latestFile.parsing = {
                        engine: engine || (file.type === 'application/pdf' ? 'pdftotext' : 'tesseract'),
                        parsedAt: new Date(this.now()).toISOString(),
                        errorCode: unavailable ? 'parser_unavailable' : 'unreadable_document',
                        missingFields: ['readable document text'],
                        remediation: message
                    };
                }
            });
            throw Object.assign(new Error(message), { status: unavailable ? 503 : 422 });
        } finally {
            try { fs.rmSync(tempDir, { recursive: true, force: true }); } catch {}
        }
        return this.update((latestDb) => {
            const latestFile = latestDb.files.find((item) => item.id === id && item.userId === userId);
            if (!latestFile) throw Object.assign(new Error('File was deleted while parsing.'), { status: 409 });
            latestFile.parsing = { engine, parsedAt: new Date(this.now()).toISOString(), ...extractFields(text, latestFile.expectedModel) };
            latestFile.status = latestFile.parsing.modelMatch === false
                ? 'model_mismatch'
                : latestFile.parsing.missingFields.length >= 4 ? 'parsed_incomplete' : 'parsed';
            return latestFile;
        });
    }
    ...
    cleanupExpiredFiles() {
        const db = this.read();
        const expired = db.files.filter((item) => Date.parse(item.expiresAt) <= this.now());
        ...
        if (expired.length) {
            ...
            this.update((latestDb) => {
                latestDb.files = latestDb.files.filter((item) => !ids.has(item.id));
            });
        }
    ...
    verifyBackup() {
        if (!fs.existsSync(this.databaseBackupFile)) return { available: false, valid: false, error: 'No backup snapshot has been created yet.' };
        try {
            const backup = readJson(this.databaseBackupFile, null);
            const valid = backup && ['users', 'assessments', 'files'].every((key) => Array.isArray(backup[key]));
            return { available: true, valid: Boolean(valid), error: valid ? '' : 'Backup snapshot has an invalid schema.' };
        } catch (error) {
            return { available: true, valid: false, error: error.message };
        }
    }

    createBackup() {
        const database = this.read();
        atomicJson(this.databaseBackupFile, database);
        return { created: true, file: this.databaseBackupFile, users: database.users.length, assessments: database.assessments.length, files: database.files.length };
    }

    restoreBackup() {
        const status = this.verifyBackup();
        if (!status.valid) throw Object.assign(new Error(status.error || 'No valid backup snapshot is available.'), { status: 409 });
        const backup = readJson(this.databaseBackupFile, null);
        atomicJson(this.databaseFile, backup);
        const storage = this.auditStorage({ repair: true });
        return { restored: true, storage };
    }

    auditStorage({ repair = false } = {}) {
        const db = this.read();
        const diskIds = new Set(fs.readdirSync(this.fileRoot)
            .filter((name) => /^[0-9a-f-]+\.bin$/i.test(name))
            .map((name) => name.slice(0, -4)));
        const recordIds = new Set(db.files.map((item) => item.id));
        const orphanIds = [...diskIds].filter((id) => !recordIds.has(id));
        const missingIds = db.files.filter((item) => !diskIds.has(item.id)).map((item) => item.id);
        const duplicateIds = [];
        const seen = new Set();
        db.files.forEach((item) => {
            const key = `${item.userId}:${item.sha256}`;
            if (seen.has(key)) duplicateIds.push(item.id);
            else seen.add(key);
        });
        if (repair && (orphanIds.length || missingIds.length || duplicateIds.length)) {
            orphanIds.forEach((id) => { try { fs.unlinkSync(path.join(this.fileRoot, `${id}.bin`)); } catch {} });
            duplicateIds.forEach((id) => { try { fs.unlinkSync(path.join(this.fileRoot, `${id}.bin`)); } catch {} });
            const remove = new Set([...missingIds, ...duplicateIds]);
            this.update((latestDb) => { latestDb.files = latestDb.files.filter((item) => !remove.has(item.id)); });
        }
        return {
            ok: !orphanIds.length && !missingIds.length && !duplicateIds.length,
            repaired: repair && Boolean(orphanIds.length || missingIds.length || duplicateIds.length),
            orphanIds, missingIds, duplicateIds
        };
    }

    health() {
        const pdf = this.parseDocument ? { command: 'custom', available: true, resolved: 'custom parser' }
            : executableStatus(process.env.PDFTOTEXT_BIN || 'pdftotext');
        const image = this.parseDocument ? { command: 'custom', available: true, resolved: 'custom parser' }
            : executableStatus(process.env.TESSERACT_BIN || 'tesseract');
        const storage = this.auditStorage();
        const backup = this.verifyBackup();
        const disk = diskStatus(this.root, Number(process.env.CONSUMER_DISK_WARNING_PERCENT || 85));
        return {
            ok: true,
            productionReady: this.explicitSessionSecret && this.explicitFileKey && pdf.available && image.available && storage.ok && disk.ok,
            accounts: true, privateEncryptedFiles: true, retentionDays: RETENTION_DAYS,
            parsers: { pdf, image }, storage, backup, disk,
            ...
            warnings: [
                ...
                !pdf.available && `PDF parser is unavailable: ${pdf.command}.`,
                !image.available && `Image OCR is unavailable: ${image.command}.`,
                !storage.ok && 'Private file storage is inconsistent.',
                !disk.ok && (disk.error || `Persistent storage usage is ${disk.usedPercent}%.`),
                !backup.valid && backup.error,
                !process.env.OPENAI_API_KEY && 'OPENAI_API_KEY is not configured; the optional assistant is disabled.'
            ].filter(Boolean)
        };
    }
}

module.exports = { ConsumerService, MAX_FILE_BYTES, MAX_FILES_PER_ACCOUNT, detectType, extractFields, executableStatus, hashPassword, verifyPassword };
