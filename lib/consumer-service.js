...
function extractFields(text, expectedModel = '') {
    ...
    const standards = [...new Set(compact.match(/(?:EN|IEC|ISO|FCC|ETSI|UL|ASTM)\s*[A-Z0-9-]*(?:\s*\d{2,6}(?:[-:.]\d+)*)?|UN\s*38\.3/gi) || [])].slice(0, 12);
    ...
    const missingFields = [
        ...
        documentKind === 'FCC' && !fccId && 'FCC ID',
        documentKind === 'UN38.3' && !batteryModel && 'battery model'
    ].filter(Boolean);
    return {
...
module.exports = { ConsumerService, MAX_FILE_BYTES, MAX_FILES_PER_ACCOUNT, detectType, extractFields, hashPassword, verifyPassword };
