...
const ALLOWED_EXTENSIONS = new Set(['.xlsx', '.xls', '.csv', '.tsv', '.txt', '.html', '.htm', '.json']);
...
function decodeArtifact({ fileName, contentBase64 }) {
    ...
    if (!buffer.length || buffer.length > MAX_ARTIFACT_BYTES) throw new Error('Artifact must be between 1 byte and 15 MB');
