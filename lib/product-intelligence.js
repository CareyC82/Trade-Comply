...
const ATTRIBUTE_RULES = [
    ...
    },
    {
        ...
        confidence: 0.9,
        pattern: /\b(hbm|hbm2e|hbm3e?|dram|ddr[3-6]?|lpddr[3-6]?|gddr[5-7]?|nand|flash\s*memory|memory\s*(chip|ic|module|stack|controller)|ssd\s*controller|storage\s*controller|controller\s*ic|storage\s*ic)\b/i,
        expansion: ['memory chip', 'HBM', 'DRAM', 'NAND flash', 'semiconductor'],
        reason: 'Memory IC language implies semiconductor classification, bandwidth/performance review, origin evidence, and export-control screening.'
    },
    {
        id: 'ai_chip',
        vertical: 'semiconductor',
        confidence: 0.92,
        pattern: /\b(ai\s*(gpu|chip|accelerator)|gpu|advanced computing|inference accelerator|h100|h200|h800|a100|a800|b100|b200|l40s?|mi300x?|gb200|rtx\s*pro)\b/i,
...
function classifyMemorySubtype(description = '') {
    const text = normalizeText(description).toLowerCase();
    if (/\b(hbm|hbm2e|hbm3e?|high\s*bandwidth\s*memory)\b/.test(text)) {
        return {
            id: 'hbm',
            label: 'HBM / high-bandwidth memory',
            risk: 'critical',
            expansion: ['HBM', 'high bandwidth memory', 'memory bandwidth', 'AI accelerator memory', 'advanced computing memory'],
            reason: 'HBM language implies high-bandwidth AI hardware exposure and should be screened before route or tax optimization.'
        };
    }
    if (/\b(dram|ddr[3-6]?|lpddr[3-6]?|gddr[5-7]?|memory\s*module|dimm|sodimm)\b/.test(text)) {
        return {
            id: 'dram',
            label: 'DRAM / memory module',
            risk: 'high',
            expansion: ['DRAM', 'DDR memory', 'LPDDR', 'memory module', 'density and bandwidth review'],
            reason: 'DRAM and memory-module language implies density, bandwidth, origin, and end-use screening.'
        };
    }
    if (/\b(nand|flash\s*memory|3d\s*nand)\b/.test(text)) {
        return {
            id: 'nand',
            label: 'NAND / flash memory',
            risk: 'medium-high',
            expansion: ['NAND flash', '3D NAND', 'flash memory', 'storage IC', 'data-center storage'],
            reason: 'NAND language implies storage IC classification, origin, and data-center or sensitive end-use review.'
        };
    }
    if (/\b(ssd\s*controller|storage\s*controller|memory\s*controller|controller\s*ic)\b/.test(text)) {
        return {
            id: 'ssd_controller',
            label: 'SSD controller / storage controller IC',
            risk: 'high',
            expansion: ['SSD controller', 'storage controller IC', 'memory controller', 'firmware and encryption review'],
            reason: 'Controller IC language should be separated from finished SSD media because firmware, encryption, and controller origin can drive risk.'
        };
    }
    return null;
}
...
const PRODUCT_EXPANSION_RULES = [
    ...
    {
        vertical: 'electronics',
        pattern: /\b(bluetooth speaker|wireless speaker|wireless microphone|wireless mic|lavalier microphone)\b/i,
        expansion: ['consumer electronics', 'wireless', 'radio equipment', 'audio equipment', 'battery']
    },
    {
        vertical: 'electronics',
        pattern: /\b(smart plug|smart outlet|smart socket|smart light|smart bulb|wifi plug|wifi light|bluetooth led light)\b/i,
        expansion: ['consumer electronics', 'wireless', 'radio equipment', 'mains electrical safety', 'connected device']
    },
    {
        vertical: 'electronics',
        pattern: /\b(wireless keyboard|bluetooth keyboard|wireless mouse|bluetooth mouse|gaming? controller|gamepad|usb hub|docking station)\b/i,
        expansion: ['computer peripheral', 'consumer electronics', 'wireless', 'radio equipment', 'battery']
    },
    {
        vertical: 'electronics',
        pattern: /\b(e-?reader|ebook reader|kindle|mini projector|portable projector|usb fan|rechargeable fan|electric shaver|cordless shaver)\b/i,
        expansion: ['consumer electronics', 'powered product', 'electrical safety', 'battery', 'wireless']
    },
    {
        vertical: 'semiconductor',
...
function inferProductAttributes(description = '', context = {}) {
    ...
    if (precheckIds.includes('ai_chip') || precheckIds.includes('memory_ic') || precheckIds.includes('advanced_manufacturing')) {
        precheckIds.push('semiconductor');
    }
    const memorySubtype = precheckIds.includes('memory_ic') ? classifyMemorySubtype(text) : null;
    if (memorySubtype) {
        uniqueAppend(expansionTerms, memorySubtype.expansion);
        reasons.push(memorySubtype.reason);
    }
