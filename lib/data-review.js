...
const { validateRiskSignal, riskSignalToTag } = require('./risk-signal');
 ...
 */
function stagePendingItems({ tags = [], cases = [], risk_signals = [], meta = {}, source = 'policy-tracker' } = {}) {
    ...
    for (const rawSignal of risk_signals) {
        const validation = validateRiskSignal(rawSignal);
        if (!validation.ok) {
            skipped.push({ kind: 'risk_signal', reason: validation.errors.join('; ') });
            continue;
        }
        const signal = validation.value;
        const signalId = signal.signal_id;
        if (prodTagIds.has(signalId) || pendingIds.tagIds.has(signalId)) {
            skipped.push({ kind: 'risk_signal', signal_id: signalId, reason: 'Already in prod or pending queue' });
            continue;
        }
        const item = createPendingItem({ kind: 'risk_signal', payload: signal, meta, source });
        queue.items.push(item);
        pendingIds.tagIds.add(signalId);
        staged.push(item);
    }
...
function approvePendingItem(pendingId, { rebuildCatalog: shouldRebuild = true } = {}) {
    ...
    if (item.kind === 'tag') {
        ...
        const { stampManualReviewAudit } = require('./compliance-audit');
        tags.push(stampManualReviewAudit(item.payload));
        writeProdTags(tags);
        published = { kind: 'tag', tag_id: tagId };
    } else if (item.kind === 'risk_signal') {
        const tags = loadProdTags();
        const tagPayload = riskSignalToTag(item.payload);
        const tagId = tagPayload.tag_id;
        if (tags.some(tag => tag.tag_id === tagId)) {
            queue.items.splice(found.index, 1);
            saveQueue(queue);
            return { ok: false, error: `Risk signal ${tagId} already exists in production` };
        }
        const { stampManualReviewAudit } = require('./compliance-audit');
        tags.push(stampManualReviewAudit(tagPayload));
        writeProdTags(tags);
        published = { kind: 'risk_signal', tag_id: tagId, country: tagPayload.country };
    ...
    return {
        ...
        message: catalogWarning
            ? `Approved and wrote production data, but catalog rebuild failed: ${catalogWarning}`
