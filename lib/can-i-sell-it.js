...
(function exposeCanISellIt(root, factory) {
        ...
        return files.map((file) => {
            ...
            return {
                ...
                extracted: {
                    ...
                    batteryModel: parsing.batteryModel || '', adapterModel: parsing.adapterModel || '',
                    radioModule: parsing.radioModule || '', hardwareRevision: parsing.hardwareRevision || '',
                    missingFields: parsing.missingFields || []
                },
        ...
        if (market === 'AU') return [
            {
                ...
                authority: 'Australian Border Force', reviewedAt,
            ...
            },
            {
                ...
                authority: 'Australian Anti-Dumping Commission', reviewedAt,
        ...
        if (market === 'NZ') return [
            {
                ...
                authority: 'New Zealand Customs Service', reviewedAt,
    ...
    function findDutySignal({ origin, market, hsCode, dutyRates, entryDate = new Date().toISOString().slice(0, 10) }) {
        ...
        return {
            ...
            rateLayers: {
                baseDuty: numberOrNull(override?.base_rate ?? rule.base_rate),
                sws: numberOrNull(override?.sws_rate),
                igst: numberOrNull(override?.igst_rate),
                preferenceRateText: override?.preference_rate_text || '',
                preferenceStatus: override?.preference_rate_status || ''
            },
    ...
    function buildSellerActionPlan({ coverage, procurement, supplierRequest, requirements = [], unanswered = [], platformDecision }) {
        if (!coverage.supported) return {
            ...
            next: 'Prepare a compliance readiness review request with the non-confidential product description and trade route.'
        };
    ...
    function buildComplianceReviewRecommendation({ input = {}, profile = {}, coverage = {}, supplierEvidence = [], documentGaps = [], requirements = [], verdict = '' } = {}) {
        const description = String(input.description || '');
        const rangeRequested = /\b(?:multiple\s+)?(?:skus?|models?|product\s+range|assortment|catalog(?:ue)?)\b/i.test(description);
        const evidenceSubmitted = supplierEvidence.length > 0 || Boolean(input.supplierEvidence?.supplierModel || input.supplierEvidence?.documentText);
        let code = 'single_product_review';
        let title = 'Import Compliance Readiness Review';
        let service = 'Single Product Review';
        let reason = 'Review the likely compliance path, exact-model evidence gaps and practical next actions before ordering or shipping.';
        if (rangeRequested) {
            code = 'product_range_review';
            service = 'Product Range Review';
            reason = 'Several products or models need a scoped review so shared evidence and SKU-specific gaps can be separated.';
        } else if (evidenceSubmitted || supplierEvidence.some((item) => item.status === 'suspected_mismatch')) {
            code = 'supplier_document_review';
            service = 'Supplier Document Review';
            reason = 'Review whether the supplier reports, declarations, model references, manufacturer and component evidence are consistent.';
        } else if (!coverage.supported) {
            reason = 'This product is outside maintained automated coverage. A human review is needed before drawing a market-access conclusion.';
        } else if (verdict === 'high_risk' || requirements.some((item) => item.severity === 'high')) {
            reason = 'Specialist confirmation is recommended because the identified product facts can trigger higher-risk requirements.';
        } else if (documentGaps.length) {
            reason = `${documentGaps.length} supplier evidence gap${documentGaps.length === 1 ? '' : 's'} should be checked against the exact model before ordering or listing.`;
        }
        const anzElectrical = ['AU', 'ANZ'].includes(input.market)
            && (profile.mainsPowered === true || profile.bundledAdapter === true || profile.bluetooth === true || profile.wifi === true || profile.cellular === true || profile.radioTransmitter === true);
        return {
            code,
            title,
            service,
            reason,
            marketMessage: anzElectrical
                ? 'Importing electrical products into Australia? Get an RCM & EESS readiness review before ordering or shipping.'
                : '',
            requestedInputs: ['Product name and images', 'Exact model and specification sheet', 'Power, wireless, battery and adaptor details', 'Manufacturer, brand and available reports / declarations', 'Origin, destination and sales channel'],
            deliverables: ['Preliminary product and regulatory scope', 'Market-specific compliance path and responsible parties', 'Exact-model supplier evidence consistency check', 'Red / Amber / Green / Unknown gap list', 'Recommended next actions and preliminary cost / timing range', 'Items requiring laboratory, certification body, customs or professional confirmation'],
            boundary: 'TraceWize provides preliminary compliance screening and evidence-readiness review, not certification or legal advice. It does not issue RCM approvals, certifications or registrations, and cannot guarantee approval, customs clearance, platform acceptance or market access.'
        };
    }

    function buildEvidenceConsistencyMatrix({ profile = {}, supplierEvidence = [] } = {}) {
        const hasFiles = supplierEvidence.length > 0;
        const valuesFor = (key) => supplierEvidence
            .map((item) => item.extracted?.[key])
            .flatMap((value) => Array.isArray(value) ? value : [value])
            .map((value) => String(value || '').trim())
            .filter(Boolean);
        const distinctReferences = (key) => [...new Set(valuesFor(key).map(normalizedReference).filter(Boolean))];
        const statusForCheck = (key) => {
            const values = supplierEvidence.map((item) => item.checks?.[key]).filter((value) => value !== undefined);
            if (values.includes(false)) return 'Mismatch';
            if (values.includes(true)) return 'Match';
            return hasFiles ? 'Unverified' : 'Missing';
        };
        const extractedStatus = (key, applicable = true, allowDifferentValues = false) => {
            if (!applicable) return 'Not applicable';
            const values = valuesFor(key);
            if (!values.length) return 'Missing';
            if (!allowDifferentValues && distinctReferences(key).length > 1) return 'Mismatch';
            if (values.length > 1 && distinctReferences(key).length === 1) return 'Match';
            return 'Unverified';
        };
        const checkedReferenceStatus = (checkKey, extractKey) => {
            const checked = statusForCheck(checkKey);
            if (checked === 'Mismatch') return checked;
            if (distinctReferences(extractKey).length > 1) return 'Mismatch';
            if (checked === 'Match') return checked;
            if (valuesFor(extractKey).length) return 'Unverified';
            return 'Missing';
        };
        const hasRadio = profile.bluetooth === true || profile.wifi === true || profile.cellular === true || profile.radioTransmitter === true;
        return [
            { key: 'product_model', label: 'Product model', status: checkedReferenceStatus('model', 'model'), note: 'Must identify the exact ordered and shipped model across every file.' },
            { key: 'manufacturer', label: 'Manufacturer / certificate holder', status: checkedReferenceStatus('holder', 'manufacturer'), note: 'Conflicting legal names require documentary explanation.' },
            { key: 'standards', label: 'Test standards and versions', status: extractedStatus('standards', true, true), note: 'Different tests may cite different standards; each edition and scope still requires review.' },
            { key: 'battery', label: 'Battery model', status: extractedStatus('batteryModel', profile.battery === true || profile.coinBattery === true), note: 'Cell and pack evidence must match the shipped battery.' },
            { key: 'adapter', label: 'Adaptor / power-supply model', status: extractedStatus('adapterModel', profile.mainsPowered === true || profile.bundledAdapter === true), note: 'An external adaptor is reviewed separately from the host product.' },
            { key: 'radio_module', label: 'Wireless module and antenna configuration', status: extractedStatus('radioModule', hasRadio), note: 'Module, antenna, bands and host conditions must remain consistent.' },
            { key: 'hardware', label: 'Hardware revision', status: extractedStatus('hardwareRevision'), note: 'A changed revision can invalidate otherwise matching evidence.' }
        ];
    }

    function buildReviewPreparationStatus({ profile = {}, service = '', reviewDetails = {} } = {}) {
        const hasRadio = profile.bluetooth === true || profile.wifi === true || profile.cellular === true || profile.radioTransmitter === true;
        const expected = [
            ['exactModel', 'exact model'],
            ['productNotes', 'product description, image notes or a public product link'],
            [(profile.mainsPowered === true || profile.bundledAdapter === true) && 'power', 'power and adaptor details'],
            [hasRadio && 'wireless', 'wireless functions'],
            [(profile.battery === true || profile.coinBattery === true) && 'battery', 'battery details'],
            ['documents', 'available document types (or state that none are available)'],
            [service === 'Product Range Review' && 'skuCount', 'number of SKUs']
        ].filter(([key]) => key);
        const missing = expected.filter(([key]) => !String(reviewDetails[key] || '').trim()).map(([, label]) => label);
        return {
            complete: missing.length === 0,
            completed: expected.length - missing.length,
            total: expected.length,
            missing,
            message: missing.length
                ? `${expected.length - missing.length} of ${expected.length} useful details prepared. Still helpful: ${missing.join('; ')}.`
                : `All ${expected.length} useful details are prepared. Review the email draft before sending.`
        };
    }

    function buildComplianceReviewBrief({ profile = {}, coverage = {}, verdict = '', requirements = [], supplierEvidence = [], documentGaps = [], nextActions = [] } = {}) {
        const evidence = buildEvidenceConsistencyMatrix({ profile, supplierEvidence });
        const hasMismatch = evidence.some((item) => item.status === 'Mismatch');
        const overall = !coverage.supported ? 'Unknown'
            : verdict === 'high_risk' || hasMismatch ? 'Red'
                : documentGaps.length ? 'Amber'
                    : 'Green';
        return {
            overall,
            scope: { status: coverage.supported ? 'Amber' : 'Unknown', detail: coverage.supported ? 'Maintained preliminary coverage; exact-model applicability remains to be confirmed.' : 'Outside maintained automated coverage.' },
            path: { status: verdict === 'high_risk' ? 'Red' : requirements.length ? 'Amber' : 'Unknown', detail: requirements.length ? `${requirements.length} preliminary requirement${requirements.length === 1 ? '' : 's'} identified.` : 'No reliable regulatory path identified.' },
            evidence,
            actions: nextActions.slice(0, 5),
            costTiming: { status: 'Unknown', detail: 'Cost and timing require human scoping plus laboratory, authority and service-provider confirmation.' },
            professionalConfirmation: requirements
                .filter((item) => item.severity === 'high' || item.requirementClass === 'scope_check' || item.coverageLevel === 'monitor_professional_confirmation_required')
                .map((item) => item.title)
        };
    }

    function buildReviewContact({ description = '', origin = '', market = '', platform = '', productLabel = '', resultLabel = '', reviewService = 'Single Product Review', reviewDetails = {} } = {}) {
        const email = 'carey@tracewize.com';
        const subject = `${reviewService}: ${productLabel || 'product pre-check'}`;
        const detailLines = [
            ['Exact model', reviewDetails.exactModel],
            ['Product / image notes', reviewDetails.productNotes],
            ['Power and adaptor', reviewDetails.power],
            ['Wireless functions', reviewDetails.wireless],
            ['Battery', reviewDetails.battery],
            ['Available document types', reviewDetails.documents],
            ['Number of SKUs', reviewDetails.skuCount]
        ].filter(([, value]) => String(value || '').trim()).map(([label, value]) => `${label}: ${String(value).trim().slice(0, 500)}`);
        const preparedDetails = detailLines.length ? `\nPrepared non-confidential details:\n${detailLines.join('\n')}\n` : '';
        const text = `Compliance readiness review request\n\nRequested service: ${reviewService}\nProduct: ${description}\nMade in: ${origin}\nTarget market: ${market}\nSales channel: ${platform}\nPreliminary result: ${resultLabel}\n${preparedDetails}\nThis non-confidential summary intentionally excludes supplier identity, pricing and uploaded files. Nothing has been submitted automatically.`;
    ...
    function assess(input) {
        ...
        const serviceRecommendation = buildComplianceReviewRecommendation({
            input, profile, coverage: coverageStatus, supplierEvidence, documentGaps, requirements, verdict
        });
        const complianceReviewBrief = buildComplianceReviewBrief({
            profile, coverage: coverageStatus, verdict, requirements, supplierEvidence, documentGaps,
            nextActions: [
                unanswered.length ? 'Confirm the remaining product facts.' : 'Lock the exact product specification.',
                'Collect the exact-model supplier evidence.',
                'Confirm classification, applicable standards and responsible party.',
                'Obtain professional confirmation for every Red or Unknown item.'
            ]
        });
        return {
            verdict,
    ...
    function assessAnz(input) {
        ...
        const serviceRecommendation = buildComplianceReviewRecommendation({
            input: { ...input, market: 'ANZ' }, profile: primary.profile, coverage: primary.coverageStatus,
            supplierEvidence: primary.supplierEvidence, documentGaps, requirements, verdict: primary.verdict
        });
        const complianceReviewBrief = buildComplianceReviewBrief({
            profile: primary.profile, coverage: primary.coverageStatus, verdict: primary.verdict,
            requirements, supplierEvidence: primary.supplierEvidence, documentGaps,
            nextActions: ['Confirm the exact product specification for both markets.', 'Separate shared evidence from Australia and New Zealand local obligations.', 'Obtain professional confirmation for every Red or Unknown item.']
        });
        return {
            ...primary,
            marketTargets: ['AU', 'NZ'],
    ...
    return {
        ...
        buildDecisionTrace, buildSellerActionPlan, buildComplianceReviewRecommendation, buildEvidenceConsistencyMatrix, buildComplianceReviewBrief, buildReviewPreparationStatus,
