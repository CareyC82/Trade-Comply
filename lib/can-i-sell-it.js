...
(function exposeCanISellIt(root, factory) {
    ...
    function extractProfile(description) {
        ...
        return {
            ...
            ,
            mainsPowered: detect(/\b(ac input|mains|wall plug|\d{2,3}\s*v(?:ac|~)?|100-?240v|power adapter)\b/i, /battery only|usb only|no\s+(mains|ac input)/i),
            bundledAdapter: detect(/\b(power|ac)\s+adapt(?:e|o)r\s+(included|bundled|supplied)\b/i, /\b(no|without)\s+(power|ac)\s+adapt(?:e|o)r\b/i),
            ratedVoltage: voltageMatch ? Number(voltageMatch[1]) : null,
            ratedPower: powerMatch ? Number(powerMatch[1]) : null
        };
    }
    ...
    function requirement(id, title, reason, docs, severity = 'required', sourceIds = []) {
        const scopeChecks = new Set(['jp_pse', 'jp_product_safety', 'sg_safety', 'sg_general_safety']);
    ...
    function marketRequirements(market, profile) {
        ...
        const scopeDetail = (mapping) => {
            ...
            const checks = [];
            if (mapping.maxVoltage) checks.push(profile.ratedVoltage == null ? `rated voltage not confirmed (category limit ${mapping.maxVoltage} VAC)` : `${profile.ratedVoltage} V ${profile.ratedVoltage <= mapping.maxVoltage ? 'is within' : 'exceeds'} the ${mapping.maxVoltage} VAC category limit`);
            if (mapping.maxPower) checks.push(profile.ratedPower == null ? `rated power not confirmed (category limit ${mapping.maxPower} W)` : `${profile.ratedPower} W ${profile.ratedPower <= mapping.maxPower ? 'is within' : 'exceeds'} the ${mapping.maxPower} W category limit`);
        ...
        if (market === 'US') {
            ...
            if (!jpScopeResult.excluded && (profile.mainsPowered === true || ['charger', 'power_bank'].includes(profile.productType) || (jpScope?.trigger === 'mains_not_false' && profile.mainsPowered !== false))) {
            ...
            if (!sgScopeResult.excluded && (profile.mainsPowered === true || profile.productType === 'charger' || (sgScope?.trigger === 'mains_not_false' && profile.mainsPowered !== false))) {
