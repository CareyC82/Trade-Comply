...
(function exposeCanISellIt(root, factory) {
    ...
    function buildEvidenceConsistencyMatrix({ profile = {}, supplierEvidence = [] } = {}) {
        ...
        const extractedStatus = (key, applicable = true) => {
            if (!applicable) return 'Not applicable';
            if (supplierEvidence.some((item) => item.status === 'suspected_mismatch' || item.status === 'verification_failed')) return 'Mismatch';
            if (supplierEvidence.some((item) => item.extracted?.[key]?.length || String(item.extracted?.[key] || '').trim())) return 'Unverified';
            return 'Missing';
        };
        const hasRadio = profile.bluetooth === true || profile.wifi === true || profile.cellular === true || profile.radioTransmitter === true;
        return [
            { key: 'product_model', label: 'Product model', status: statusForCheck('model'), note: 'Must identify the exact ordered and shipped model.' },
            { key: 'manufacturer', label: 'Manufacturer / certificate holder', status: statusForCheck('holder'), note: 'Legal-name differences require documentary explanation.' },
            { key: 'standards', label: 'Test standards and versions', status: extractedStatus('standards'), note: 'Presence is not proof that the correct standard edition or scope applies.' },
    ...
    return {
        ...
        buildDecisionTrace, buildSellerActionPlan, buildComplianceReviewRecommendation, buildEvidenceConsistencyMatrix, buildComplianceReviewBrief,
