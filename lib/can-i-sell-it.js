...
(function exposeCanISellIt(root, factory) {
    ...
    function buildConsumerConclusion({ requirements, unanswered, evidenceAnswers = {}, supplierEvidence = [] }) {
        ...
        const reviewedKinds = new Set(supplierEvidence
            .filter((item) => ['model_matched', 'server_parsed'].includes(item.status))
            .map((item) => item.kind));
    ...
    return {
        ...
        evidenceQuestionDefinitions, evidenceQuestionsForRequirements, buildAssessmentMatrix, marketRequirements,
