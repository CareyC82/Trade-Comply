...
(function exposeCanISellIt(root, factory) {
    ...
    function buildProcurementDecision({ verdict, documentGaps, economics, unanswered, supplierEvidence = [] }) {
        ...
        if (documentGaps.length) {
            return { code: 'conditional_buy', label: 'Purchase only after conditions are met', reason: 'Require the missing model-specific documents in the purchase contract before paying the balance.' };
        }
    ...
    function assess(input) {
        ...
        const procurement = buildProcurementDecision({ verdict, documentGaps, economics, unanswered, supplierEvidence });
