...
(function exposeCanISellIt(root, factory) {
    ...
    function analyzeSupplierEvidence({ files = [], supplierModel = '', requiredModel = '', documentText = '' } = {}) {
        ...
        const modelMatch = !target ? 'not_provided'
            : (supplier === target || text.includes(target) ? 'matched' : supplier ? 'mismatch' : 'unverified');
        return files.map((file) => {
            const name = String(file.name || '').toLowerCase();
            const type = String(file.type || '');
            const supported = /pdf|image/.test(type) || /\.(pdf|png|jpe?g|webp)$/i.test(name);
            const kind = /un.?38|battery.?test/.test(name) ? 'UN38.3'
            ...
            return {
                ...
                status: file.status === 'model_mismatch' ? 'suspected_mismatch'
                    ...
                    : modelMatch === 'mismatch' ? 'suspected_mismatch'
                        : modelMatch === 'matched' ? 'model_matched'
                            : 'unable_to_verify',
                modelMatch,
                note: file.note || (!supported
                    ? 'Unsupported file type.'
                    : modelMatch === 'matched'
                        ? 'Model text matches the entered product model; document authenticity, completeness and issuing body still require verification.'
                        : modelMatch === 'mismatch'
    ...
    function buildConsumerConclusion({ requirements, unanswered, evidenceAnswers = {} }) {
        const highRisk = requirements.find((item) => item.severity === 'high');
        if (highRisk) {
            return {
                code: 'specialist_review',
                answer: 'SPECIALIST REVIEW',
                label: 'Do not list or order this product yet',
                reason: `${highRisk.title} must be resolved by a qualified specialist.`,
        ...
        const evidence = Object.entries(evidenceAnswers).map(([key, item]) => ({
            key,
        ...
        if (unknown.length || !evidence.length) {
            const missing = unknown.length
                ? unknown.map((item) => item.label)
                : ['Exact-model compliance evidence'];
            return {
                code: 'unable_to_confirm',
                answer: 'UNABLE TO CONFIRM',
        ...
        return {
            code: 'yes_precheck',
            answer: 'YES — PRE-CHECK PASSED',
            label: 'You can proceed to listing preparation',
            reason: 'The answers indicate that the key product and exact-model evidence gates are satisfied.',
            missing: []
        };
    }

    function buildProcurementDecision({ marketDecision, verdict, requirements, documentGaps, economics, unanswered, supplierEvidence = [] }) {
        if (supplierEvidence.some((item) => item.status === 'suspected_mismatch')) {
            return { code: 'change_supplier', label: 'Change supplier or demand corrected evidence', reason: 'At least one uploaded file appears to reference a different model.' };
        }
        if (verdict === 'high_risk') {
            return { code: 'do_not_buy_yet', label: 'Do not purchase yet', reason: 'A medical, child-directed, or other specialist gate must be resolved before committing inventory.' };
        }
        if (unanswered.length) {
            return { code: 'information_needed', label: 'Do not place the order yet', reason: 'Material product facts are still unknown.' };
        }
        if (economics && economics.complete && economics.marginRate < 0) {
            return { code: 'not_recommended', label: 'Not recommended to purchase', reason: 'The entered selling price does not cover the estimated landed and channel costs.' };
        }
        if (economics && economics.complete && economics.marginRate < 0.15) {
            return { code: 'renegotiate', label: 'Renegotiate or change supplier', reason: 'The estimated gross contribution is below 15%, leaving little room for returns, ads, or price changes.' };
        }
        if (documentGaps.length) {
        ...
        return { code: 'ready_for_po_review', label: 'Suitable for purchase-order review', reason: 'No blocking gap was identified from the answers, but final model and tariff verification is still required.' };
    }
