...
(function exposeCanISellIt(root, factory) {
    ...
    function coverageForProfile(profile) {
    ...
    const marketEvidenceKinds = {
        US: new Set(['FCC', 'UN38.3', 'SDS', 'RoHS']),
        EU: new Set(['CE / RED', 'RoHS', 'UN38.3', 'SDS']),
    ...
    function analyzeSupplierEvidence({ files = [], supplierModel = '', requiredModel = '', supplierName = '', documentText = '', market = '' } = {}) {
        ...
        return files.map((file) => {
            ...
            return {
                ...
                kind,
                status: file.status === 'model_mismatch' || checks.model === false ? 'suspected_mismatch'
        ...
        questions.forEach((question) => {
            ...
            if (!(question.kinds || []).length && answer !== true) {
                items.push({
                    ...
                    reason: answer === false ? 'the platform approval was reported unavailable' : 'platform approval was not confirmed'
                });
                return;
            }
    ...
    function buildSellerConclusion({ coverage, consumerConclusion }) {
    ...
    function assess(input) {
        ...
        const coverageStatus = coverageForProfile(profile);
        ...
        const sellerConclusion = buildSellerConclusion({ coverage: coverageStatus, consumerConclusion });
    ...
    return {
        UNKNOWN, detectProductType, coverageForProfile, extractProfile, materialQuestionKeys, getFollowUpQuestions,
        ...
        analyzeSupplierEvidence, buildSupplierRequest,
        buildDecisionTrace,
