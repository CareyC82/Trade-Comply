    ...
    function marketRequirements(market, profile) {
        ...
        if (market === 'US') {
            ...
            if (profile.mainsPowered === true || ['charger', 'power_bank'].includes(profile.productType)) {
                requirements.push(requirement('jp_pse', 'Japan PSE scope and importer review', 'Covered AC adapters, electrical appliances and secondary batteries require product-scope, importer, inspection and marking checks; covered overseas direct sellers also need the applicable Japanese notification and domestic-responsible-person setup.', ['PSE scope rationale', 'Conformity/test evidence', 'Importer or overseas-seller notification records', 'Domestic responsible-person details where applicable', 'PSE label artwork'], 'required', ['jpPse', 'jpOnlineSeller']));
            }
