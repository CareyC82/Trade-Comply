...
(function (root, factory) {
    ...
    function describeRateTrust(rule = {}, checkedAt = '', now = Date.now()) {
        ...
        const freshness = getRateFreshness(checkedAt, now);
    ...
    function buildTariffRow(rule = {}, dutyRates = {}, override = null) {
        ...
        const row = {
            ...
            rateLayers: {
                baseDuty: baseRate,
                sws: override?.sws_rate ?? null,
                igst: override?.igst_rate ?? null,
            ...
            effectiveFrom: override?.effective_from || rule.effective_from || '',
            effectiveTo: override?.effective_to || rule.effective_to || '',
    ...
    function isEffectiveOn(row = {}, asOfDate) {
        const day = String(asOfDate || new Date().toISOString()).slice(0, 10);
        const start = String(row.effective_from || '').slice(0, 10);
        const end = String(row.effective_to || '').slice(0, 10);
        return (!start || start <= day) && (!end || end >= day);
    }

    function buildTariffRows(dutyRates = {}, options = {}) {
        ...
        asArray(dutyRates.rules).forEach((rule) => {
            ...
            }).filter((override) => isEffectiveOn(override, asOfDate));
    ...
    function buildUpcomingTariffRows(dutyRates = {}, asOfDate = new Date().toISOString().slice(0, 10)) {
        const day = String(asOfDate).slice(0, 10);
        return asArray(dutyRates.rules).flatMap((rule) => asArray(rule.exact_code_overrides)
            .filter((row) => String(row.effective_from || '').slice(0, 10) > day)
            .map((row) => buildTariffRow(rule, dutyRates, row)))
            .sort((left, right) => left.effectiveFrom.localeCompare(right.effectiveFrom));
    }

    function buildCurrentTariffRows(dutyRates = {}, limit = 10, asOfDate = new Date().toISOString().slice(0, 10)) {
        const rows = buildTariffRows(dutyRates, { asOfDate });
    ...
    function buildTariffWatchModel({ syncStatus = {}, dutyRates = {}, limit = 8, asOfDate = new Date().toISOString().slice(0, 10) } = {}) {
        const counts = syncStatus.counts || {};
        const diagnostics = syncStatus.ci_diagnostics || {};
        const rateChanges = toNumber(counts.total_rate_changes || syncStatus.automation_digest?.rate_change_count);
        ...
        return {
            ...
            impactedRoutes,
            asOfDate,
            currentTariffRows: buildCurrentTariffRows(dutyRates, 12, asOfDate),
            marketTariffRows: buildTariffRows(dutyRates, { asOfDate }),
            upcomingTariffRows: buildUpcomingTariffRows(dutyRates, asOfDate),
