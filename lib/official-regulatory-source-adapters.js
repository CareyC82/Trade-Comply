...
function stripHtml(html) {
    const main = String(html || '').match(/<(?:main|article)\b[^>]*>([\s\S]*?)<\/(?:main|article)>/i)?.[1] || String(html || '');
    return main
...
function parseOfficialPayload({ source, body, contentType = '' }) {
    ...
    if (adapter === 'official_pdf_fingerprint') {
        ...
        return { ok: true, adapter, content: `${source.authority}. ${source.title}. Official PDF bytes ${buffer.length}. SHA256 ${fingerprint}.`, binaryHash: fingerprint };
    }
