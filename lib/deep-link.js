...
function getInboundDeepLinkFromSearch(searchString = '') {
    ...
    return { query, direction, country };
}
...
function normalizeInboundCountry(value) {
    ...
    const allowed = new Set(['US', 'EU', 'ASEAN', 'RU', 'TW', 'JP', 'KR', 'GLOBAL', 'OTHER']);
...
function buildInboundSearchPlan(searchString) {
    const { query, direction, country } = getInboundDeepLinkFromSearch(searchString);
    if (!query) {
        return {
            shouldRun: false,
            query: '',
