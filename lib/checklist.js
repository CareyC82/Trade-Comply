...
/** Read phase/stage from checklist item — never confuse rule `category` (e.g. EXPORT_CTRL) with phase. */
...
function normalizeChecklistItem(item, source = 'library') {
    ...
    const phaseLabel = getPhaseDisplayLabel(rawPhase);
...
function groupChecklistByPhase(items) {
    ...
    (items || []).forEach((item) => {
        const rawPhase = extractRawPhaseFromItem(item) || item.phase;
        const phaseKey = normalizePhase(rawPhase || item);
        const phaseLabel = item.phaseLabel || getPhaseDisplayLabel(rawPhase || item);
