...
function buildSnapshot({ sources, previous = {}, fetched = {}, now = new Date().toISOString() }) {
    ...
    Object.entries(sources).forEach(([id, source]) => {
        ...
        if (prior && prior.content_hash !== row.content_hash) changes.push({ id, type: 'content_changed', previous_hash: prior.content_hash, current_hash: row.content_hash, detected_at: now });
