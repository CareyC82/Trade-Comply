...
function manualReviewIsCurrent(source, nowMs = Date.now()) {
    const policy = source.monitorPolicy || {};
    if (!['last_good_manual_review', 'automatic_with_manual_fallback'].includes(policy.mode)) return false;
    const reviewed = Date.parse(source.reviewedAt || '');
    const intervalDays = Number(policy.reviewEveryDays || policy.review_every_days || 30);
    return Number.isFinite(reviewed) && Number.isFinite(intervalDays)
        && reviewed + intervalDays * 24 * 60 * 60 * 1000 >= nowMs;
}
...
function capturedBinaryHash(value = '') {
    return String(value).match(/\bSHA256\s+([a-f0-9]{64})\b/i)?.[1]?.toLowerCase() || '';
}

function classifyChange(change, source, currentRow) {
    if (change.type !== 'content_changed') return 'substantive_candidate';
    const previousBinaryHash = capturedBinaryHash(change.previous_summary);
    const currentBinaryHash = capturedBinaryHash(change.current_summary || currentRow?.content_summary);
    if (previousBinaryHash && previousBinaryHash === currentBinaryHash) return 'monitor_metadata_enrichment';
...
function buildSnapshot({ sources, previous = {}, fetched = {}, now = new Date().toISOString() }) {
    ...
    Object.entries(sources).forEach(([id, source]) => {
        ...
        if (response === undefined && prior?.content_hash) {
            ...
            const manuallyCurrent = manualReviewIsCurrent(source, nowMs);
            row = {
                ...prior, id, url: source.url, fetched_at: now,
                status: manuallyCurrent ? 'manual_review_current' : 'last_good_degraded',
                preserved_last_good: true,
                manually_reviewed_at: manuallyCurrent ? source.reviewedAt : prior.manually_reviewed_at,
...
module.exports = { normalizeContent, contentHash, metadataSeed, contentIdentityMatches, capturedBinaryHash, classifyChange, lifecycleState, manualReviewIsCurrent, buildSnapshot };
