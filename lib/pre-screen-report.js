...
function buildTopTriggers(tags) {
    return prioritizeTags(tags).slice(0, 5).map((tag) => {
        const detail = String(tag.short_description || tag.content_en || tag.description || '').trim();
        const clippedDetail = detail.length > 220 ? `${detail.slice(0, 220).trim()}...` : detail;
        return {
            title: String(tag.short_name || tag.source_citation || tag.tag_id || 'Matched rule').trim(),
...
function buildActionPlan(missingInformation = [], verificationObjects = []) {
    const seen = new Set();
    const add = (phase, priority, text) => {
        const task = normalizeActionText(text);
        if (!task || seen.has(task.toLowerCase())) {
            return null;
        }
        ...
        return {
            ...
            detail: priority === 'critical'
                ? 'Resolve this before relying on the pre-check result or proceeding with shipment.'
                : 'Keep this in the review pack for broker, counsel, or internal compliance review.'
        };
    };

    const critical = missingInformation
        .slice(0, 4)
        .map((item) => add('Before shipment', 'critical', item))
        .filter(Boolean);
    const recommended = verificationObjects
        .slice(0, 4)
        .map((item) => add('Verification pack', 'recommended', item))
        .filter(Boolean);

    return [...critical, ...recommended].slice(0, 8);
}

function buildRiskDrivers(profile = {}, tags = []) {
    const drivers = [];
    const seen = new Set();
    const add = (label, source = 'attribute') => {
        ...
        seen.add(key);
        drivers.push({ label: clean, source });
    };

    (profile.selectedAttributeLabels || []).forEach((label) => add(label, 'attribute'));
    prioritizeTags(tags).slice(0, 4).forEach((tag) => {
        const category = String(tag.category || '').replace(/_/g, ' ').toLowerCase();
        if (category && category !== 'other') {
            add(category, 'rule');
        }
        if (tag.jurisdiction || tag.country) {
            add(`${tag.jurisdiction || tag.country} rule`, 'jurisdiction');
        }
    });
...
function buildPreScreenReport(context = {}) {
    ...
    const topTriggers = buildTopTriggers(tags);
    const missingInformation = buildMissingInformationChecklist(context, profile);
    const verificationObjects = buildVerificationObjects(context, tags, profile);
    ...
    return {
        ...
        risk_drivers: buildRiskDrivers(profile, tags),
        top_triggers: topTriggers,
        trigger_reason: triggerReason,
        action_plan: buildActionPlan(missingInformation, verificationObjects),
        missing_information: missingInformation,
        verification_objects: verificationObjects,
