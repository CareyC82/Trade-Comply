...
describe('high-risk product journeys', () => {
    for (const sample of highRiskSet.products) {
        it(`keeps ${sample.id} classified, sourced, and route-aware`, () => {
            ...
            assert.equal(model.routeComparison.length, 3, `${sample.id} should show direct route plus two transit routes`);
            assert.equal(model.routeComparison[0].routeKind, 'direct', `${sample.id} should keep the selected route first`);
            assert.equal(model.routeComparison[0].market, sample.to, `${sample.id} selected route should match the requested destination`);
            assert.equal(model.routeComparison.filter((row) => row.routeKind === 'transit').length, 2, `${sample.id} should show exactly two transit alternatives`);
            assert.equal(model.transitRoutes.length, 2, `${sample.id} should expose exactly two transit route rows`);
            assert.ok(model.routeRecommendation?.decision, `${sample.id} should produce a route recommendation`);
            assert.ok(model.routeRecommendation.headline, `${sample.id} should produce a route recommendation headline`);
            assert.ok(model.transitRoutes.every((row) => row.transitComparison?.combinedRate), `${sample.id} transit routes should show combined rate status`);
            assert.ok(model.transitRoutes.every((row) => row.transitComparison?.firstLegRate), `${sample.id} transit routes should show first-leg duty/tax status`);
            assert.ok(model.transitRoutes.every((row) => row.transitComparison?.secondLegRate), `${sample.id} transit routes should show second-leg duty/tax status`);
            assert.ok(model.transitRoutes.every((row) => row.transitComparison?.combinedCostPer1000), `${sample.id} transit routes should show combined cost per $1k`);
            assert.ok(model.transitRoutes.every((row) => row.transitComparison?.deltaCostPer1000), `${sample.id} transit routes should show direct-route delta cost per $1k`);
            assert.ok(model.transitRoutes.every((row) => row.originSavingsCaveat === 'Potential savings only if origin transformation is legally supported.'), `${sample.id} transit routes should carry origin-transformation caveat`);
